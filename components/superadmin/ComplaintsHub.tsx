...
export const ComplaintsHub: React.FC<ComplaintsHubProps> = ({
  ...
  const openReview = (complaint: EmployeeComplaint) => {
    ...
    // Pre-fill existing values if editing an already-acknowledged complaint
    setReviewAction(complaint.actionTaken && complaint.actionTaken !== 'NONE' ? complaint.actionTaken : suggested);
    setJudgment(complaint.judgment || '');
    setResolution(complaint.resolution || '');
    setViewComplaint(null);
    playSound('click');
  };

  const handleQuickAcknowledge = async (complaintId: string) => {
    const c = complaints.find(x => x.id === complaintId);
    if (!c) return;
    setViewComplaint(null);
    setIsProcessing(complaintId);
    try {
      const { error } = await supabase
        ...
        .update({
          [DB_COLUMNS.STATUS]: 'ACKNOWLEDGED',
          [DB_COLUMNS.REVIEWED_BY]: reviewerName,
          [DB_COLUMNS.REVIEWED_AT]: getTrueISOString(),
        })
        .eq(DB_COLUMNS.ID, complaintId);
      if (error) throw error;
      await logAudit({
        activityType: 'COMPLAINT_ACKNOWLEDGED',
        entityType: 'EMPLOYEE_REPORT',
        description: `Complaint for ${c.employeeName} — Acknowledged (pending judgment)`,
        branchId: c.branchId,
        performerName: reviewerName,
      });
      playSound('success');
  ...
  const handleReview = async () => {
    ...
    // Keep existing status if already acknowledged, otherwise promote to ACKNOWLEDGED
    const newStatus = complaint.status === 'ACKNOWLEDGED' ? 'ACKNOWLEDGED' : 'ACKNOWLEDGED';
    setReviewState(null);
    ...
    try {
      ...
      await logAudit({
        activityType: 'COMPLAINT_JUDGMENT_ADDED',
        entityType: 'EMPLOYEE_REPORT',
        description: `Complaint for ${complaint.employeeName} — Judgment added (${reviewAction})`,
                ...
                <div className="border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 px-5 py-4 shrink-0">
                  {c.status === 'PENDING' ? (
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <button
                        disabled={isCurrentlyProcessing}
                        onClick={() => handleQuickAcknowledge(c.id)}
                        className="h-12 rounded-2xl bg-emerald-600 dark:bg-emerald-500 text-white text-xs font-semibold uppercase tracking-wide hover:bg-emerald-700 dark:hover:bg-emerald-600 active:scale-95 transition-all disabled:opacity-40"
                      >
                        Acknowledge
                      </button>
                    </div>
                    ...
                    <div className="space-y-2">
                      <button
                        ...
                        className="w-full h-12 rounded-2xl bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 text-xs font-semibold uppercase tracking-wide hover:bg-slate-800 dark:hover:bg-white active:scale-95 transition-all disabled:opacity-40"
                      >
                        {c.actionTaken && c.actionTaken !== 'NONE' ? 'Edit Judgment' : 'Add Judgment'}
                      </button>
                      <div className="flex items-center gap-2">
                        <button
                          disabled={isCurrentlyProcessing}
                        ...
                        <button
                          ...
                          className="flex-1 h-10 rounded-2xl border-2 border-amber-200 dark:border-amber-500/30 bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 text-xs font-semibold uppercase tracking-wide hover:bg-amber-100 dark:hover:bg-amber-500/20 active:scale-95 transition-all disabled:opacity-40 flex items-center justify-center gap-2"
                        >
          ...
          <div className="bg-white dark:bg-slate-900 rounded-t-[28px] sm:rounded-2xl w-full sm:max-w-md shadow-xl overflow-hidden animate-in slide-in-from-bottom-4 sm:zoom-in-95 duration-200 flex flex-col max-h-[92vh]" onClick={e => e.stopPropagation()}>
            ...
            <div className="bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 px-6 pt-6 pb-5 shrink-0">
              <div className="flex items-start justify-between gap-3 mb-3">
                <div>
                  <p className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wide mb-1">{reviewState.complaint.status === 'ACKNOWLEDGED' ? 'Add / Edit Judgment' : 'Review Complaint'}</p>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-sm shadow-xl overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            <div className="p-7 text-center space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 pt-1">
                ...
                <button
                  onClick={() => { setReviewConfirm(false); handleReview(); }}
