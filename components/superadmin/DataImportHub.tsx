    ...
    <div className="max-w-3xl mx-auto space-y-6 sm:space-y-8 pb-32 px-2">
      <div className="bg-white p-6 sm:p-8 md:p-12 rounded-2xl sm:rounded-[56px] border border-slate-100 shadow-sm space-y-8 sm:space-y-10">
        <div className="text-center space-y-2">
          <div className="w-12 h-12 sm:w-16 sm:h-16 bg-white border border-slate-200 rounded-2xl sm:rounded-3xl flex items-center justify-center mx-auto text-xl sm:text-2xl mb-4 sm:mb-6 shadow-xl">📥</div>
        ...
        {!file ? (
          <button 
            ...
            className="w-full py-12 sm:py-24 rounded-2xl sm:rounded-3xl border-4 border-dashed border-slate-100 bg-slate-50 flex flex-col items-center justify-center gap-4 sm:gap-6 hover:border-emerald-500 transition-all group disabled:opacity-50 shadow-inner"
          >
            ...
            <p className="text-xs sm:text-sm font-medium text-slate-400 uppercase tracking-wider">Select .SQL Migration Script</p>
          </button>
        ) : (
          <div className="space-y-4 sm:space-y-6 animate-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
              {summary.map(table => (
                <div key={table.tableName} className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 bg-slate-50 rounded-[16px] sm:rounded-xl border border-slate-100 shadow-sm">
        ...
        <div className="pt-2 sm:pt-4 space-y-4 sm:space-y-6">
          ...
          <button 
            ...
            className={`w-full py-5 sm:py-7 rounded-xl sm:rounded-2xl font-bold uppercase tracking-wider sm:tracking-wide text-xs sm:text-sm shadow-xl transition-all active:scale-[0.98] ${isProcessing || summary.length === 0 ? 'bg-slate-50 text-slate-300' : 'bg-slate-950 text-white hover:bg-emerald-700'}`}
          >
            {isProcessing ? (
