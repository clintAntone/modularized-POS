...
import { Branch, BranchBill, BillPayment, BillStatus, BillsCatalogItem, VaultCarryover } from '../../types';
...
import {
  ...
  BookOpen, Layers, Vault, Check, AlertTriangle
} from 'lucide-react';
...
type HubTab = 'catalog' | 'branches' | 'carryover';
...
type CarryoverForm = { amount: string; effectiveDate: string; notes: string; recordedBy: string };
const emptyCarryoverForm = (): CarryoverForm => ({
  amount: '', effectiveDate: getManilaToday(), notes: '', recordedBy: ''
});

// ── Main component ────────────────────────────────────────
export const BillsCatalogHub: React.FC<BillsCatalogHubProps> = ({ branches }) => {
  ...
  const [carryoverTarget, setCarryoverTarget] = useState<Branch | null>(null);
  const [carryoverForm, setCarryoverForm] = useState<CarryoverForm>(emptyCarryoverForm);
  const [deletingCarryover, setDeletingCarryover] = useState<VaultCarryover | null>(null);
  const [expandedCarryoverBranch, setExpandedCarryoverBranch] = useState<string | null>(null);
  ...
  // ── Queries ──────────────────────────────────────────────
  const { data: catalog = [], refetch: refetchCatalog } = useQuery<BillsCatalogItem[]>({
  ...
  const { data: allCarryover = [], refetch: refetchCarryover } = useQuery<VaultCarryover[]>({
    queryKey: [DB_TABLES.VAULT_CARRYOVER],
    queryFn: async () => {
      const { data, error } = await supabase
  ...
  // CARRYOVER HANDLERS
  // ─────────────────────────────────────────────────────────
  const handleOpenCarryover = (branch: Branch) => {
    playSound('click');
    setCarryoverTarget(branch);
    setCarryoverForm(emptyCarryoverForm());
    setShowCarryoverModal(true);
  };

  const handleSaveCarryover = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!carryoverTarget || isSubmitting) return;
    const amt = parseFloat(carryoverForm.amount);
    if (!amt) { showToast('Enter a valid amount', 'error'); return; }
    setIsSubmitting(true);
    try {
      const { error } = await supabase.from(DB_TABLES.VAULT_CARRYOVER).insert({
        branch_id: carryoverTarget.id,
        amount: amt,
        effective_date: carryoverForm.effectiveDate,
        notes: carryoverForm.notes.trim() || null,
        recorded_by: carryoverForm.recordedBy.trim() || null,
      });
      if (error) throw error;
      playSound('success');
      showToast(`Carryover recorded for ${carryoverTarget.name.replace('BRANCH - ', '')}`);
  ...
  const handleDeleteCarryover = async () => {
    if (!deletingCarryover || isSubmitting) return;
    setIsSubmitting(true);
    try {
      const { error } = await supabase
        .from(DB_TABLES.VAULT_CARRYOVER)
        .delete()
        .eq(DB_COLUMNS.ID, deletingCarryover.id);
      if (error) throw error;
      playSound('success');
      showToast('Entry removed');
      setDeletingCarryover(null);
      refetchCarryover();
    } catch (err: any) {
  ...
  // ── Carryover totals per branch ───────────────────────────
  const carryoverByBranch = useMemo(() => {
    const map: Record<string, VaultCarryover[]> = {};
    allCarryover.forEach(c => {
      if (!map[c.branchId]) map[c.branchId] = [];
      map[c.branchId].push(c);
    });
    return map;
  }, [allCarryover]);

  const totalNetworkCarryover = useMemo(() =>
    allCarryover.reduce((s, c) => s + c.amount, 0),
    [allCarryover]
  );
    ...
    <div className="space-y-6 animate-in fade-in duration-700">
      ...
      <div className="bg-white p-6 sm:p-8 rounded-[32px] border border-slate-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Catalog · Branch Bills · Vault Carryover</p>
          </div>
        </div>
        ...
        <div className="flex w-full sm:w-auto bg-slate-100 p-1 rounded-2xl gap-1">
          {([
            ...
            { id: 'carryover' as HubTab, label: 'Carryover', icon: <Vault className="w-3.5 h-3.5" /> },
          ] as const).map(tab => (
            <button
              key={tab.id}
      ...
      {activeTab === 'carryover' && (
        <div className="space-y-6">
          <div className="flex items-center justify-between px-1">
            <div>
              <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">Vault Opening Balances</h4>
              <p className="text-[9px] font-bold text-slate-400 mt-0.5">Cash that was in the vault before POS tracking began.</p>
            </div>
            <div className="bg-white border border-slate-200 rounded-2xl px-5 py-3 text-right shadow-sm">
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Network Total</p>
              <p className="text-xl font-black text-indigo-600 tabular-nums">₱{totalNetworkCarryover.toLocaleString()}</p>
            </div>
          </div>

          <div className="space-y-3">
            {branches.map(branch => {
              const entries = carryoverByBranch[branch.id] || [];
              const total = entries.reduce((s, e) => s + e.amount, 0);
              const isExpanded = expandedCarryoverBranch === branch.id;
                ...
                <div key={branch.id} className="bg-white rounded-[28px] border border-slate-200 shadow-sm overflow-hidden">
                  <div className="flex items-center justify-between px-6 sm:px-8 py-5">
                    <div
                      ...
                      onClick={() => {
                        if (entries.length > 0) {
                          setExpandedCarryoverBranch(prev => prev === branch.id ? null : branch.id);
                        }
                      }}
                    >
                      <div className="w-9 h-9 bg-slate-100 rounded-xl flex items-center justify-center text-slate-500 text-sm font-black uppercase shrink-0">
                      ...
                      <div className="min-w-0">
                        <p className="font-black text-slate-900 uppercase text-sm tracking-tight leading-none truncate">{branch.name.replace('BRANCH - ', '')}</p>
                        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">
                          {entries.length > 0 ? `${entries.length} entr${entries.length !== 1 ? 'ies' : 'y'}` : 'No carryover recorded'}
                        </p>
                      </div>
                      {total > 0 && (
                        <div className="ml-2 shrink-0">
                          <span className="bg-indigo-50 text-indigo-700 text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-xl tabular-nums">
                    ...
                    <div className="flex items-center gap-2 shrink-0 ml-3">
                      <button
                        onClick={() => handleOpenCarryover(branch)}
                        className="flex items-center gap-1.5 px-3 py-2 bg-slate-900 text-white rounded-xl font-black text-[9px] uppercase tracking-widest hover:bg-indigo-600 transition-all active:scale-95 shadow"
                      >
                        <Plus className="w-3 h-3" /> Record
                      </button>
                      {entries.length > 0 && (
                        <div className="w-7 h-7 rounded-xl bg-slate-100 flex items-center justify-center text-slate-400 cursor-pointer" onClick={() => setExpandedCarryoverBranch(prev => prev === branch.id ? null : branch.id)}>
                        ...
                        {entries.map(entry => (
                          <div key={entry.id} className="px-5 py-4 flex items-center justify-between gap-3">
                            <div className="min-w-0 space-y-1">
                              <p className="font-black text-indigo-600 tabular-nums text-base">₱{entry.amount.toLocaleString()}</p>
                              <p className="text-[10px] font-bold text-slate-500">
                                {new Date(entry.effectiveDate + 'T00:00:00').toLocaleDateString('en-PH', { month: 'short', day: 'numeric', year: 'numeric' })}
                                {entry.recordedBy ? ` · ${entry.recordedBy}` : ''}
                              </p>
                              {entry.notes && <p className="text-[9px] font-bold text-slate-400 normal-case">{entry.notes}</p>}
                            </div>
                            <button onClick={() => { playSound('warning'); setDeletingCarryover(entry); }} className="w-9 h-9 rounded-xl bg-rose-50 flex items-center justify-center text-rose-400 active:scale-95 transition-all shrink-0">
                              <Trash2 className="w-4 h-4" />
                            </button>
                          </div>
                        ))}
                        <div className="px-5 py-4 bg-slate-50 flex items-center justify-between">
                          <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Total Carryover</p>
                          <p className="font-black text-indigo-700 tabular-nums">₱{total.toLocaleString()}</p>
                        </div>
                      </div>
                      {/* Desktop table */}
                      <div className="hidden sm:block overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                          <thead>
                            <tr className="bg-slate-50">
                              <th className="px-8 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Effective Date</th>
                              <th className="px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Amount</th>
                              <th className="px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Notes</th>
                              <th className="px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Recorded By</th>
                              <th className="px-8 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Remove</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-slate-50">
                            {entries.map(entry => (
                              <tr key={entry.id} className="hover:bg-slate-50/50 transition-colors">
                                <td className="px-8 py-3 text-[10px] font-bold text-slate-700 tabular-nums">{new Date(entry.effectiveDate + 'T00:00:00').toLocaleDateString('en-PH', { month: 'short', day: 'numeric', year: 'numeric' })}</td>
                                <td className="px-4 py-3 text-right font-black text-indigo-600 tabular-nums text-xs">₱{entry.amount.toLocaleString()}</td>
                                <td className="px-4 py-3 text-[10px] font-bold text-slate-400 max-w-xs truncate">{entry.notes || '—'}</td>
                                <td className="px-4 py-3 text-[10px] font-bold text-slate-400">{entry.recordedBy || '—'}</td>
                                <td className="px-8 py-3 text-right"><button onClick={() => { playSound('warning'); setDeletingCarryover(entry); }} className="w-7 h-7 rounded-lg bg-rose-50 hover:bg-rose-100 flex items-center justify-center text-rose-400 transition-colors ml-auto"><Trash2 className="w-3.5 h-3.5" /></button></td>
                              </tr>
                            ))}
                          </tbody>
                          <tfoot>
                            <tr className="bg-slate-50 border-t border-slate-100">
                              <td className="px-8 py-3 text-[9px] font-black text-slate-500 uppercase tracking-widest">Total Carryover</td>
                              <td className="px-4 py-3 text-right font-black text-indigo-700 tabular-nums text-sm">₱{total.toLocaleString()}</td>
                              <td colSpan={3}></td>
                            </tr>
                          </tfoot>
                        </table>
      ...
      {/* Record vault carryover modal */}
      {showCarryoverModal && carryoverTarget && (
        <div className={UI_THEME.layout.modalWrapper} onClick={() => setShowCarryoverModal(false)}>
          <div className={`${UI_THEME.layout.modalStandard} ${UI_THEME.radius.modal} p-8 space-y-6 animate-in zoom-in-95 duration-200`} onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-start">
              <div>
                <h4 className="text-xl font-black text-slate-900 uppercase tracking-tight">Record Carryover</h4>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">{carryoverTarget.name.replace('BRANCH - ', '')}</p>
              </div>
              <button onClick={() => setShowCarryoverModal(false)} className="w-8 h-8 rounded-xl bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-500 transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="bg-indigo-50 rounded-2xl p-4">
              <p className="text-[9px] font-bold text-indigo-600 uppercase tracking-widest leading-relaxed">
                Record the vault cash that existed in this branch <strong>before POS tracking began</strong>. This amount is added to the branch's running vault balance.
              </p>
            </div>

            <form onSubmit={handleSaveCarryover} className="space-y-4">
              <div>
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-2">Amount</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-8 pr-4 py-3 bg-slate-50 border-2 border-transparent rounded-xl font-black text-slate-800 text-[13px] outline-none focus:border-indigo-500 focus:bg-white transition-all"
                  />
                </div>
              </div>
              <div>
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-2">Effective Date</label>
                <input
                  type="date" required
                  value={carryoverForm.effectiveDate}
                  onChange={e => setCarryoverForm(f => ({ ...f, effectiveDate: e.target.value }))}
                  className="w-full px-4 py-3 bg-slate-50 border-2 border-transparent rounded-xl font-bold text-slate-800 text-[12px] outline-none focus:border-indigo-500 focus:bg-white transition-all"
                />
              </div>
              <div>
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-2">Notes (optional)</label>
                <input
                  ...
                  onChange={e => setCarryoverForm(f => ({ ...f, notes: e.target.value }))}
                  placeholder="e.g. Opening balance from January operations"
                  className="w-full px-4 py-3 bg-slate-50 border-2 border-transparent rounded-xl font-bold text-slate-800 text-[11px] outline-none focus:border-indigo-500 focus:bg-white transition-all"
                />
              </div>
              <div>
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-2">Recorded By (optional)</label>
                <input
                  type="text"
                  value={carryoverForm.recordedBy}
                  onChange={e => setCarryoverForm(f => ({ ...f, recordedBy: e.target.value }))}
              ...
              <button
                type="submit"
                disabled={isSubmitting || !carryoverForm.amount}
                className="w-full bg-indigo-600 text-white font-black py-4 rounded-2xl text-[12px] uppercase tracking-widest shadow-lg active:scale-95 transition-all disabled:opacity-40 flex items-center justify-center gap-3 hover:bg-indigo-700"
              >
                {isSubmitting ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> : 'Record Carryover'}
              </button>
            </form>
          </div>
        </div>
      ...
      {/* Delete carryover entry confirm */}
      {deletingCarryover && (
        <div className={UI_THEME.layout.modalWrapper}>
          <div className={`${UI_THEME.layout.modalStandard} ${UI_THEME.radius.modal} p-10 text-center border border-slate-100 shadow-2xl`}>
            <div className="w-16 h-16 bg-rose-50 text-rose-500 rounded-2xl flex items-center justify-center mx-auto mb-6"><Trash2 className="w-8 h-8" /></div>
            <h4 className="text-2xl font-bold text-slate-900 mb-2 uppercase tracking-tighter">Remove Entry?</h4>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed">
              Remove ₱{deletingCarryover.amount.toLocaleString()} carryover entry.<br />
              This will reduce the branch's vault balance.
            </p>
            <div className="flex flex-col gap-3 mt-10">
              <button onClick={handleDeleteCarryover} disabled={isSubmitting} className="w-full bg-rose-600 text-white font-black py-5 rounded-2xl text-[12px] uppercase tracking-widest shadow-lg active:scale-95 transition-all flex items-center justify-center gap-3">
                {isSubmitting ? <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin"></div> : 'Remove Entry'}
              </button>
              <button onClick={() => setDeletingCarryover(null)} className="w-full text-slate-400 font-bold py-4 rounded-xl text-[11px] uppercase tracking-widest">Cancel</button>
            </div>
          </div>
        </div>
      )}
