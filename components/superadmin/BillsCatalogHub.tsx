                          ...
                          <tbody className="divide-y divide-slate-50">
                            {entries.map(entry => (
                              <tr key={entry.id} className="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td className="px-8 py-3 text-right">{!isReadOnly && <button onClick={() => { playSound('warning'); setDeletingCarryover(entry); }} className="w-7 h-7 rounded-lg bg-rose-50 hover:bg-rose-100 flex items-center justify-center text-rose-400 transition-colors ml-auto"><Trash2 className="w-3.5 h-3.5" /></button>}</td>
                              </tr>
                            ))}
                          </tbody>
                          <tfoot>
