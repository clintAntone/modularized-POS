      ...
      <div className="bg-slate-900 rounded-[24px] px-5 py-5 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-sm font-black uppercase tracking-tight text-white leading-none truncate">Portal Users</h2>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-0.5">Limited dashboard accounts</p>
          </div>
          <button
            onClick={openCreate}
          ...
          >
            ...
            <span className="text-xs font-black text-white uppercase tracking-widest">New User</span>
          </button>
        </div>
        <div className="grid grid-cols-3 gap-2">
          ...
          <div className="bg-white/5 rounded-2xl px-4 py-3">
            <p className="text-xs font-black text-slate-500 uppercase tracking-widest mb-1">Active</p>
            <p className={`text-2xl font-black tabular-nums leading-none ${activeCount > 0 ? 'text-emerald-400' : 'text-white'}`}>{activeCount}</p>
          </div>
          <div className="bg-white/5 rounded-2xl px-4 py-3">
            <p className="text-xs font-black text-slate-500 uppercase tracking-widest mb-1">Admins</p>
      ...
      {isLoading ? (
        ...
        <div className="bg-white border border-slate-100 rounded-[24px] overflow-hidden shadow-sm">
          <div className="px-5 py-3.5 bg-slate-900 flex items-center justify-between gap-3">
            <p className="text-xs font-black text-white uppercase tracking-widest shrink-0">Accounts</p>
            <p className="text-xs font-bold text-slate-500 uppercase tracking-widest shrink-0 ml-auto">
          ...
          <div className="px-4 py-3 border-b border-slate-50 flex items-center gap-2">
            ...
            <div className="relative shrink-0">
              <button
                ...
                className={`h-9 px-3 flex items-center gap-1.5 border rounded-xl text-xs font-black uppercase tracking-widest transition-all ${tabFilter.length > 0 ? 'bg-slate-900 text-white border-slate-900' : 'bg-slate-50 border-slate-100 text-slate-600 hover:bg-slate-100'}`}
              >
              ...
              {tabDropdownOpen && (
                <div className="absolute right-0 top-10 z-50 w-52 bg-white border border-slate-100 rounded-2xl shadow-xl overflow-hidden">
                  ...
                  <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-50">
                    <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Filter by Tab</p>
                    {tabFilter.length > 0 && (
                      <button
                        type="button"
                        ...
                        {TAB_DEFINITIONS.filter(t => t.category === cat).map(t => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <span className={`text-xs font-black uppercase tracking-widest ${checked ? 'text-slate-900' : 'text-slate-500'}`}>{t.label}</span>
                            </button>
                          );
                        })}
                      </div>
                ...
                <div key={u.id} className={`bg-white border border-slate-100 rounded-2xl shadow-sm flex flex-col transition-all ${!u.isActive ? 'opacity-50' : ''}`}>
                  ...
                  >
                    ...
                    <div className="px-4 pb-4 flex flex-wrap gap-1.5">
                      {u.isSuperadmin ? (
                        <span className="text-xs font-black uppercase tracking-widest px-2 py-1 rounded-lg bg-amber-50 text-amber-600 border border-amber-100">Full Admin</span>
                      ) : (
                        <span className={`text-xs font-black uppercase tracking-widest px-2 py-1 rounded-lg border ${isReadOnly ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100'}`}>
                          {isReadOnly ? 'Read Only' : 'Read+Write'}
                        </span>
                      )}
                      {!u.isSuperadmin && (
                        <>
                          <span className="text-xs font-bold text-slate-400 uppercase tracking-widest px-2 py-1 rounded-lg bg-slate-50 border border-slate-100 truncate max-w-[110px]">
                            {label ?? 'All branches'}
                          </span>
                          <span className="text-xs font-bold text-slate-400 uppercase tracking-widest px-2 py-1 rounded-lg bg-slate-50 border border-slate-100">
                            {count} tab{count !== 1 ? 's' : ''}
                          </span>
                        </>
                      )}
            ...
            <div className="bg-slate-900 px-6 pt-5 pb-4 rounded-t-3xl shrink-0">
              ...
              <div className="grid grid-cols-2 gap-1 bg-white/10 rounded-2xl p-1">
                <button
                  ...
                  className={`h-8 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${formTab === 'account' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-white'}`}
                >
                  Account
                </button>
                <button
                  ...
                  className={`h-8 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${formTab === 'permissions' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-white'}`}
                >
                  Permissions
                </button>
              </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="bg-slate-50 border border-slate-200 rounded-2xl overflow-hidden divide-y divide-slate-200">
                      ...
                      <div className="flex items-center h-12 px-3.5 gap-3">
                        <label className="text-xs font-black uppercase tracking-widest text-slate-400 w-24 shrink-0">Display Name</label>
                        <input
                          type="text"
                      ...
                      {/* Username */}
                      <div className="flex items-center h-12 px-3.5 gap-3">
                        <label className="text-xs font-black uppercase tracking-widest text-slate-400 w-24 shrink-0">Username</label>
                        <input
                          type="text"
                          value={form.username}
                      ...
                      {/* PIN */}
                      <div className="flex items-center h-12 px-3.5 gap-3">
                        <label className="text-xs font-black uppercase tracking-widest text-slate-400 w-24 shrink-0">
                      ...
                      {(form.pin || !editingId) && (
                        <div className="flex items-center h-12 px-3.5 gap-3">
                          <label className="text-xs font-black uppercase tracking-widest text-slate-400 w-24 shrink-0">Confirm</label>
                          <input
                            type="password"
                            inputMode="numeric"
                    ...
                    {editingId && !form.pin && (
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-widest px-1">Blank PIN = keep current</p>
                    )}
                  </div>
                    ...
                    <div className="bg-slate-50 border border-slate-200 rounded-2xl overflow-hidden divide-y divide-slate-200">
                      ...
                      <div className="flex items-center justify-between px-3.5 py-2.5">
                        <div className="min-w-0 mr-3">
                          <p className="text-xs font-black uppercase tracking-widest text-slate-700 leading-none">Full Admin</p>
                      ...
                      {editingId && (
                        <div className="flex items-center justify-between px-3.5 py-2.5">
                          <div className="min-w-0 mr-3">
                            <p className="text-xs font-black uppercase tracking-widest text-slate-700 leading-none">Account Active</p>
                    ...
                    {!form.isSuperadmin && (
                      <div className="space-y-1.5 pt-0.5">
                        ...
                        <div className="grid grid-cols-2 gap-1.5">
                          ...
                          >
                            <span className="text-xs font-black uppercase tracking-widest">Read Only</span>
                          </button>
                          <button
                            type="button"
                          ...
                          >
                            <span className="text-xs font-black uppercase tracking-widest">Read+Write</span>
                          </button>
                        </div>
                        {form.readOnly && (
                ...
                {formTab === 'permissions' && <div className="px-5 py-4 space-y-4">
                  ...
                  {form.isSuperadmin ? (
                    <div className="flex flex-col items-center justify-center py-16 gap-3 opacity-30">
                      ...
                      <p className="text-xs font-black text-slate-400 uppercase tracking-widest text-center">Full Admin — all permissions granted</p>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                        <div>
                          <p className="text-xs font-black uppercase tracking-widest text-slate-500">Tab Access</p>
                            ...
                            <div key={cat} className="rounded-xl border border-slate-100 overflow-hidden">
                              ...
                              <div className="flex items-center bg-slate-50">
                                ...
                                >
                                  ...
                                  <span className="text-xs font-black text-slate-500 uppercase tracking-[0.2em]">{cat}</span>
                                  <span className={`text-xs font-black uppercase tracking-widest px-2 py-0.5 rounded-full ml-1 ${grantedCount === catTabs.length ? 'bg-emerald-100 text-emerald-600' : grantedCount === 0 ? 'bg-slate-100 text-slate-400' : 'bg-amber-100 text-amber-600'}`}>
                                ...
                                <div className="flex items-center gap-0.5 pr-2">
                                  ...
                                  <button
                                    ...
                                    className="text-xs font-black text-slate-400 uppercase tracking-widest hover:text-slate-600 px-1.5 py-1 rounded hover:bg-slate-100 transition-colors"
                                  >None</button>
                                </div>
                              </div>
                                  ...
                                  {catTabs.map(t => {
                                    ...
                                    return (
                                      <button
                                        ...
                                        className={`h-9 px-3 rounded-xl text-xs font-bold uppercase tracking-widest transition-all flex items-center gap-2 text-left ${granted ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-400 border border-slate-200 hover:border-slate-300'}`}
                                      >
                      ...
                      <div className="space-y-3 pt-1">
                        ...
                        <div className="flex items-center justify-between px-3.5 py-3 bg-indigo-50 rounded-xl border border-indigo-100">
                          <div className="min-w-0 mr-3">
                            <p className="text-xs font-black uppercase tracking-widest text-indigo-800 leading-none">Restrict to Branches</p>
                        ...
                        {form.restrictBranches && (
                          <div className="space-y-2">
                                  ...
                                  {form.branchIds.map(id => {
                                    ...
                                    return (
                                      <button
                                        ...
                                        className="flex items-center gap-1.5 bg-indigo-600 text-white pl-2.5 pr-2 py-1 rounded-lg text-xs font-black uppercase tracking-widest active:scale-95 transition-all"
                                      >
                                        {short}
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-slate-200 text-xs">·</span>
                              <button type="button" onClick={() => setForm(p => ({ ...p, branchIds: [] }))} className="text-xs font-black text-slate-400 uppercase tracking-widest hover:text-slate-600 px-1">None</button>
                            </div>
                                    ...
                                    {filteredBranches.map(branch => {
                                      ...
                                      return (
                                        ...
                                        >
                                          ...
                                          <p className={`text-xs font-black uppercase tracking-widest truncate leading-none ${selected ? 'text-white' : 'text-slate-700'}`}>{shortName}</p>
                                        </button>
                                      );
                                    })}
                                  </div>
      ...
      {deleteConfirmId && (
        <div className={UI_THEME.layout.modalWrapper}>
          <div className="bg-white rounded-3xl w-full max-w-sm mx-auto shadow-2xl border border-slate-100 p-8 text-center">
            ...
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest leading-relaxed mb-8">This portal user will be permanently removed.</p>
            <div className="space-y-2">
              <button
