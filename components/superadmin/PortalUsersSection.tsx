    ...
    <div className="space-y-5 pb-4">
      ...
      <div className="bg-white border border-slate-100 rounded-2xl px-5 py-5 shadow-sm space-y-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-2xl bg-indigo-50 flex items-center justify-center shrink-0">
            <svg className="w-4 h-4 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.2"><path d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
          </div>
          <div className="flex-1 min-w-0">
            <h2 className="text-sm font-bold text-slate-900 leading-none truncate">Portal Users</h2>
            <p className="text-xs font-medium text-slate-400 mt-0.5">Limited dashboard accounts</p>
          </div>
          <button
            onClick={openCreate}
            className="shrink-0 h-9 px-4 bg-indigo-600 hover:bg-indigo-700 active:scale-95 transition-all rounded-xl flex items-center gap-1.5"
          >
            ...
            <span className="text-xs font-semibold text-white">New User</span>
          </button>
        </div>
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-slate-50 rounded-xl px-4 py-3">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1">Total</p>
            <p className="text-2xl font-black text-slate-900 tabular-nums leading-none">{users.length}</p>
          </div>
          <div className="bg-slate-50 rounded-xl px-4 py-3">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1">Active</p>
            <p className={`text-2xl font-black tabular-nums leading-none ${activeCount > 0 ? 'text-emerald-600' : 'text-slate-900'}`}>{activeCount}</p>
          </div>
          <div className="bg-slate-50 rounded-xl px-4 py-3">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1">Admins</p>
            <p className={`text-2xl font-black tabular-nums leading-none ${adminCount > 0 ? 'text-amber-600' : 'text-slate-900'}`}>{adminCount}</p>
          </div>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white border border-slate-100 rounded-2xl py-14 text-center shadow-sm">
          <p className="text-xs font-bold text-slate-300 uppercase tracking-widest">No portal users yet</p>
          <p className="text-xs font-bold text-slate-200 uppercase tracking-widest mt-1">Create accounts for owners or stakeholders</p>
        </div>
      ) : (
        <div className="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm">
          <div className="px-5 py-3.5 bg-slate-50 border-b border-slate-100 flex items-center justify-between gap-3">
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide shrink-0">Accounts</p>
            <p className="text-xs font-medium text-slate-400 shrink-0 ml-auto">
          ...
          <div className="px-4 py-3 border-b border-slate-50 flex items-center gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full h-9 pl-8 pr-3 bg-slate-50 border border-slate-100 rounded-xl text-xs font-black text-slate-700 uppercase tracking-widest placeholder:font-bold placeholder:tracking-widest placeholder:text-slate-300 outline-none focus:border-slate-300 focus:bg-white transition-all"
              />
            </div>
            ...
            <div className="relative shrink-0">
              <button
                ...
                className={`h-9 px-3 flex items-center gap-1.5 border rounded-xl text-xs font-semibold uppercase tracking-wide transition-all ${tabFilter.length > 0 ? 'bg-slate-900 text-white border-slate-900' : 'bg-slate-50 border-slate-100 text-slate-600 hover:bg-slate-100'}`}
              >
              ...
              {tabDropdownOpen && (
                <div className="absolute right-0 top-10 z-50 w-52 bg-white border border-slate-100 rounded-2xl shadow-xl overflow-hidden">
                  ...
                  <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-50">
                    <p className="text-xs font-medium text-slate-400 uppercase tracking-wide">Filter by Tab</p>
                    {tabFilter.length > 0 && (
                      <button
                        type="button"
                  ...
                  <div className="max-h-64 overflow-y-auto py-1">
                    {CATEGORIES.map(cat => (
                      <div key={cat}>
                        <p className="px-4 pt-2.5 pb-1 text-xs font-black text-slate-300 uppercase tracking-widest">{cat}</p>
                        {TAB_DEFINITIONS.filter(t => t.category === cat).map(t => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <span className={`text-xs font-semibold uppercase tracking-wide ${checked ? 'text-slate-900' : 'text-slate-500'}`}>{t.label}</span>
                            </button>
                          );
                        })}
                      </div>
          ...
          {filteredUsers.length === 0 ? (
            <div className="py-10 text-center">
              <p className="text-xs font-bold text-slate-300 uppercase tracking-widest">No users match your filter</p>
            </div>
          ) : (
                ...
                <div key={u.id} className={`bg-white border border-slate-100 rounded-2xl shadow-sm flex flex-col transition-all ${!u.isActive ? 'opacity-50' : ''}`}>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 px-4 pt-4 pb-3">
                      <div className={`relative w-9 h-9 rounded-xl flex items-center justify-center shrink-0 font-black text-xs italic select-none ${
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          <p className="text-xs font-black text-slate-900 uppercase tracking-tight leading-none truncate">{u.displayName}</p>
                        ...
                        <p className="text-xs font-bold text-slate-400 tracking-widest mt-0.5">@{u.username}</p>
                      </div>
                    </div>
                    ...
                    <div className="px-4 pb-4 flex flex-wrap gap-1.5">
                      {u.isSuperadmin ? (
                        <span className="text-xs font-semibold uppercase tracking-wide px-2 py-1 rounded-lg bg-amber-50 text-amber-600 border border-amber-100">Full Admin</span>
                      ) : (
                        <span className={`text-xs font-semibold uppercase tracking-wide px-2 py-1 rounded-lg border ${isReadOnly ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100'}`}>
                          {isReadOnly ? 'Read Only' : 'Read+Write'}
                        </span>
                      )}
                      {!u.isSuperadmin && (
                        <>
                          <span className="text-xs font-medium text-slate-400 uppercase tracking-wide px-2 py-1 rounded-lg bg-slate-50 border border-slate-100 truncate max-w-[110px]">
                            {label ?? 'All branches'}
                          </span>
                          <span className="text-xs font-medium text-slate-400 uppercase tracking-wide px-2 py-1 rounded-lg bg-slate-50 border border-slate-100">
                            {count} tab{count !== 1 ? 's' : ''}
                          </span>
                        </>
                      )}
      ...
      {showForm && (
        <div className={UI_THEME.layout.modalWrapper}>
          <form
            onSubmit={handleSave}
            className="bg-white rounded-3xl w-full max-w-lg mx-auto shadow-xl border border-slate-100 max-h-[92vh] flex flex-col"
          >
            {/* FORM HEADER — dark slate */}
            <div className="bg-white border-b border-slate-100 px-6 pt-5 pb-4 rounded-t-3xl shrink-0">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-9 h-9 rounded-2xl bg-indigo-50 flex items-center justify-center shrink-0">
                  <svg className="w-4 h-4 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.2">
              ...
              {/* Tab switcher */}
              <div className="grid grid-cols-2 gap-1 bg-slate-100 rounded-2xl p-1">
                <button
                  type="button"
                  onClick={() => setFormTab('account')}
                  className={`h-8 rounded-xl text-xs font-semibold uppercase tracking-wide transition-all ${formTab === 'account' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-white'}`}
                >
                  Account
                </button>
                <button
                  ...
                  className={`h-8 rounded-xl text-xs font-semibold uppercase tracking-wide transition-all ${formTab === 'permissions' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-white'}`}
                >
                  Permissions
                </button>
              </div>
              ...
              <div>
                ...
                {formTab === 'account' && <div className="px-5 py-4 flex flex-col gap-3">
                  ...
                  <div className="space-y-1.5">
                    <p className="text-xs font-black text-slate-400 uppercase tracking-[0.25em] px-1">Account</p>
                    <div className="bg-slate-50 border border-slate-200 rounded-2xl overflow-hidden divide-y divide-slate-200">
                      ...
                      <div className="flex items-center h-12 px-3.5 gap-3">
                        <label className="text-xs font-semibold uppercase tracking-wide text-slate-400 w-24 shrink-0">Display Name</label>
                        <input
                          type="text"
                      ...
                      {/* Username */}
                      <div className="flex items-center h-12 px-3.5 gap-3">
                        <label className="text-xs font-semibold uppercase tracking-wide text-slate-400 w-24 shrink-0">Username</label>
                        <input
                          type="text"
                          value={form.username}
                      ...
                      {/* PIN */}
                      <div className="flex items-center h-12 px-3.5 gap-3">
                        <label className="text-xs font-semibold uppercase tracking-wide text-slate-400 w-24 shrink-0">
                        ...
                        <input
                          ...
                          placeholder="••••••"
                          maxLength={6}
                          className="flex-1 h-full bg-transparent font-bold text-sm text-slate-900 outline-none tracking-wide placeholder:text-slate-300 text-right"
                        />
                      </div>
                      {/* Confirm PIN — only when typing a new PIN */}
                      {(form.pin || !editingId) && (
                        <div className="flex items-center h-12 px-3.5 gap-3">
                          <label className="text-xs font-semibold uppercase tracking-wide text-slate-400 w-24 shrink-0">Confirm</label>
                          <input
                            type="password"
                            inputMode="numeric"
                    ...
                    {editingId && !form.pin && (
                      <p className="text-xs font-medium text-slate-400 uppercase tracking-wide px-1">Blank PIN = keep current</p>
                    )}
                  </div>

                  {/* ── SETTINGS GROUP ── */}
                  <div className="flex-1 space-y-1.5">
                    <p className="text-xs font-black text-slate-400 uppercase tracking-[0.25em] px-1">Settings</p>
                    <div className="bg-slate-50 border border-slate-200 rounded-2xl overflow-hidden divide-y divide-slate-200">
                      {/* Full Admin */}
                      <div className="flex items-center justify-between px-3.5 py-2.5">
                        <div className="min-w-0 mr-3">
                          <p className="text-xs font-semibold uppercase tracking-wide text-slate-700 leading-none">Full Admin</p>
                          <p className="text-xs text-slate-400 mt-0.5 leading-none">Unrestricted access</p>
                        </div>
                        <button
                          type="button"
                      ...
                      {editingId && (
                        <div className="flex items-center justify-between px-3.5 py-2.5">
                          <div className="min-w-0 mr-3">
                            <p className="text-xs font-semibold uppercase tracking-wide text-slate-700 leading-none">Account Active</p>
                            <p className="text-xs text-slate-400 mt-0.5 leading-none">Can log in</p>
                          </div>
                          <button
                            type="button"
                    ...
                    {!form.isSuperadmin && (
                      <div className="space-y-1.5 pt-0.5">
                        <p className="text-xs font-black text-slate-400 uppercase tracking-[0.25em] px-1">Access Level</p>
                        <div className="grid grid-cols-2 gap-1.5">
                          ...
                          >
                            <span className="text-xs font-semibold uppercase tracking-wide">Read Only</span>
                          </button>
                          <button
                            type="button"
                          ...
                          >
                            <span className="text-xs font-semibold uppercase tracking-wide">Read+Write</span>
                          </button>
                        </div>
                        {form.readOnly && (
                          <p className="text-xs font-bold text-amber-600 uppercase tracking-widest leading-snug px-1">
                ...
                {formTab === 'permissions' && <div className="px-5 py-4 space-y-4">
                  <p className="text-xs font-black text-slate-400 uppercase tracking-[0.25em]">Permissions</p>
                  ...
                  {form.isSuperadmin ? (
                    <div className="flex flex-col items-center justify-center py-16 gap-3 opacity-30">
                      ...
                      <p className="text-xs font-medium text-slate-400 uppercase tracking-wide text-center">Full Admin — all permissions granted</p>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                        <div>
                          <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Tab Access</p>
                          <p className="text-xs text-slate-400 mt-0.5">Select which sections this user can view.</p>
                        </div>
                        {CATEGORIES.map(cat => {
                            ...
                            <div key={cat} className="rounded-xl border border-slate-100 overflow-hidden">
                              ...
                              <div className="flex items-center bg-slate-50">
                                ...
                                >
                                  ...
                                  <span className={`text-xs font-semibold uppercase tracking-wide px-2 py-0.5 rounded-full ml-1 ${grantedCount === catTabs.length ? 'bg-emerald-100 text-emerald-600' : grantedCount === 0 ? 'bg-slate-100 text-slate-400' : 'bg-amber-100 text-amber-600'}`}>
                                ...
                                <div className="flex items-center gap-0.5 pr-2">
                                  <button
                                    ...
                                    className="text-xs font-black text-emerald-500 uppercase tracking-widest hover:text-emerald-700 px-1.5 py-1 rounded hover:bg-emerald-50 transition-colors"
                                  >All</button>
                                  <span className="text-slate-200 text-xs">·</span>
                                  <button
                                    type="button"
                                    onClick={() => setForm(p => ({ ...p, permissions: { ...p.permissions, ...Object.fromEntries(catTabs.map(t => [t.id, false])) } }))}
                                    className="text-xs font-medium text-slate-400 uppercase tracking-wide hover:text-slate-600 px-1.5 py-1 rounded hover:bg-slate-100 transition-colors"
                                  >None</button>
                                </div>
                              </div>
                                  ...
                                  {catTabs.map(t => {
                                    ...
                                    return (
                                      <button
                                        ...
                                        className={`h-9 px-3 rounded-xl text-xs font-medium uppercase tracking-wide transition-all flex items-center gap-2 text-left ${granted ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-400 border border-slate-200 hover:border-slate-300'}`}
                                      >
                      ...
                      <div className="space-y-3 pt-1">
                        <p className="text-xs font-black text-slate-400 uppercase tracking-[0.25em]">Branch Access</p>
                        <div className="flex items-center justify-between px-3.5 py-3 bg-indigo-50 rounded-xl border border-indigo-100">
                          <div className="min-w-0 mr-3">
                            <p className="text-xs font-semibold uppercase tracking-wide text-indigo-800 leading-none">Restrict to Branches</p>
                          ...
                          <div className="space-y-2">
                            {form.branchIds.length > 0 ? (
                                  ...
                                  {form.branchIds.map(id => {
                                    ...
                                    return (
                                      <button
                                        ...
                                        className="flex items-center gap-1.5 bg-indigo-600 text-white pl-2.5 pr-2 py-1 rounded-lg text-xs font-semibold uppercase tracking-wide active:scale-95 transition-all"
                                      >
                                        {short}
                              ...
                              <div className="p-3 bg-amber-50 border border-amber-100 rounded-2xl">
                                <p className="text-xs font-black text-amber-600 uppercase tracking-widest">No branches selected — user won't be able to log in.</p>
                              </div>
                            )}
                            ...
                            <div className="flex items-center gap-2">
                              <div className="relative flex-1">
                                ...
                                <input
                                  ...
                                  className="w-full h-8 pl-8 pr-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 outline-none focus:border-indigo-300 focus:bg-white transition-all"
                                />
                              </div>
                              <button type="button" onClick={() => setForm(p => ({ ...p, branchIds: activeBranches.map(b => b.id) }))} className="text-xs font-black text-indigo-500 uppercase tracking-widest hover:text-indigo-700 px-1">All</button>
                              <span className="text-slate-200 text-xs">·</span>
                              <button type="button" onClick={() => setForm(p => ({ ...p, branchIds: [] }))} className="text-xs font-medium text-slate-400 uppercase tracking-wide hover:text-slate-600 px-1">None</button>
                            </div>

                            {activeBranches.length === 0 ? (
                              <p className="text-xs text-slate-300 text-center py-4">No active branches found.</p>
                            ) : (
                              <div className="max-h-52 overflow-y-auto rounded-xl">
                                {filteredBranches.length === 0 ? (
                                  <p className="text-xs text-slate-300 text-center py-4">No branches match "{branchSearch}"</p>
                                ) : (
                                    ...
                                    {filteredBranches.map(branch => {
                                      ...
                                      return (
                                        ...
                                        >
                                          ...
                                          <p className={`text-xs font-semibold uppercase tracking-wide truncate leading-none ${selected ? 'text-white' : 'text-slate-700'}`}>{shortName}</p>
                                        </button>
                                      );
                                    })}
                                  </div>
            ...
            <div className="px-5 pb-5 pt-3 space-y-2.5 border-t border-slate-100 shrink-0">
              {formError && (
                <div className="p-2.5 bg-rose-50 border border-rose-100 rounded-xl">
                  <p className="text-xs font-bold text-rose-600 uppercase tracking-widest">{formError}</p>
                </div>
              )}
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="h-11 px-5 text-slate-500 font-bold text-xs uppercase tracking-widest bg-slate-100 hover:bg-slate-200 rounded-2xl transition-all"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={isSaving}
                  className="flex-1 h-11 bg-slate-900 text-white font-black text-xs uppercase tracking-widest rounded-2xl hover:bg-slate-700 transition-all disabled:opacity-50 flex items-center justify-center"
                >
                  {isSaving
      ...
      {deleteConfirmId && (
        <div className={UI_THEME.layout.modalWrapper}>
          <div className="bg-white rounded-3xl w-full max-w-sm mx-auto shadow-xl border border-slate-100 p-8 text-center">
            ...
            <p className="text-xs font-medium text-slate-400 uppercase tracking-wide leading-relaxed mb-8">This portal user will be permanently removed.</p>
            <div className="space-y-2">
              <button
                onClick={() => handleDelete(deleteConfirmId)}
                className="w-full h-12 bg-rose-600 text-white font-black text-xs uppercase tracking-widest rounded-2xl hover:bg-rose-700 transition-all"
              >
                Delete
              </button>
              <button
