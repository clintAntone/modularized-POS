        ...
        <div className="flex flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Global Branch Management</p>
            </div>
          </div>
        ...
        <div className="flex flex-row items-center gap-2 sm:gap-4">
          <div className="relative flex-1 group">
            ...
            <input
                ...
                className="w-full pl-12 md:pl-14 pr-4 md:pr-6 py-3.5 md:py-4 bg-slate-50 border border-slate-200 rounded-[24px] text-xs md:text-sm font-bold uppercase tracking-widest focus:bg-white focus:border-emerald-500 focus:ring-8 focus:ring-emerald-500/5 transition-all outline-none shadow-inner placeholder:text-slate-300"
            />
          </div>

            <button
              onClick={() => { setIsFiltersOpen(!isFiltersOpen); playSound('click'); }}
              className={`flex items-center gap-2 px-4 py-2.5 md:py-4 rounded-[24px] border transition-all text-xs font-black uppercase tracking-widest shrink-0 ${isFiltersOpen ? 'bg-slate-900 text-white border-slate-900 shadow-lg' : 'bg-white text-slate-600 border-slate-200 hover:border-emerald-500 hover:text-emerald-600'}`}
            >
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-top-2 duration-300 pt-2">
            <div className="flex flex-col lg:flex-row items-stretch gap-6">
              <div className="flex-1 space-y-2">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Access Status</p>
                <div className="flex bg-slate-100 p-1 rounded-2xl border border-slate-200 shadow-inner h-14">
                  {(['all', 'active', 'inactive'] as const).map((val) => (
                    <button
                      ...
                      className={`flex-1 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${statusFilter === val ? 'bg-white text-slate-900 shadow-md scale-[1.02] border border-slate-100' : 'text-slate-400 hover:text-slate-600'}`}
                    >
              ...
              <div className="flex-1 space-y-2">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Live Status</p>
                <div className="flex bg-slate-100 p-1 rounded-2xl border border-slate-200 shadow-inner h-14">
                  {(['all', 'live', 'closed'] as const).map((val) => (
                    <button
                      key={val}
                      onClick={() => { setLiveFilter(val); playSound('click'); }}
                      className={`flex-1 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${liveFilter === val ? 'bg-white text-slate-900 shadow-md scale-[1.02] border border-slate-100' : 'text-slate-400 hover:text-slate-600'}`}
                    >
              ...
              <div className="flex-1 space-y-2">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Manager Status</p>
                <div className="flex bg-slate-100 p-1 rounded-2xl border border-slate-200 shadow-inner h-14">
                  {(['all', 'has_manager', 'no_manager'] as const).map((val) => (
                    <button
                      key={val}
                      onClick={() => { setManagerFilter(val); playSound('click'); }}
                      className={`flex-1 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${managerFilter === val ? 'bg-white text-slate-900 shadow-md scale-[1.02] border border-slate-100' : 'text-slate-400 hover:text-slate-600'}`}
                    >
      ...
      <div className="px-1 space-y-4 no-print">
        <div className="flex flex-row items-center justify-between gap-4 px-1 sm:px-2">
          ...
          <button
            ...
            className={`h-14 px-5 rounded-2xl bg-emerald-600 text-white flex items-center justify-center gap-2.5 text-xs font-black uppercase tracking-widest hover:bg-emerald-700 transition-all shadow-lg active:scale-95 shrink-0 ${isExporting ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            {isExporting ? (
            ...
            {!isExporting && <svg className="w-3 h-3 text-white/60 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3"><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>}
          </button>
        </div>
                ...
                {paginatedBranches.map(branch => (
                  ...
                  >
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {branch.faceIdEnabled === false && (
                            <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-amber-50 border border-amber-200 text-amber-600 text-xs font-black uppercase tracking-widest shrink-0">
                    ...
                    <td className="px-4 py-4 text-center">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-black uppercase tracking-widest ${
                        branch.isEnabled
                    ...
                    <td className="px-4 py-4 text-center">
                      {branch.isEnabled ? (
                        <div className="flex items-center justify-center gap-1.5">
                          ...
                          <span className={`text-xs font-black uppercase tracking-widest ${branch.isOpen ? 'text-emerald-600' : 'text-slate-300'}`}>
                    ...
                    <td className="px-4 py-4 text-center">
                      <span className="text-xs font-black text-slate-500 uppercase tracking-widest bg-slate-50 px-2 py-1 rounded-lg border border-slate-100">
              ...
              <div className="p-5 flex flex-col gap-4">
                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-black uppercase tracking-widest ${
                    ...
                    {branch.faceIdEnabled === false && (
                      <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-amber-50 border border-amber-200 text-amber-600 text-xs font-black uppercase tracking-widest">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-slate-50 rounded-xl px-3 py-2 text-center">
                    ...
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-0.5">Cutoff</p>
                  </div>
                  <div className={`rounded-xl px-3 py-2 text-center ${branch.isPinChanged ? 'bg-emerald-50' : 'bg-amber-50'}`}>
                    ...
                    <p className={`text-xs font-bold uppercase tracking-widest mt-0.5 ${branch.isPinChanged ? 'text-emerald-400' : 'text-amber-400'}`}>
