...
export const AttendanceHub: React.FC<AttendanceHubProps> = ({ branches, employees, onRefresh, isReadOnly }) => {
  ...
  const [clockOutFilters, setClockOutFilters] = useState<string[]>([]);
  const [shiftFilters, setShiftFilters] = useState<string[]>([]);
  const [filtersOpen, setFiltersOpen] = useState(false);

  const filteredAttendance = useMemo(() => {
    let result = serverAttendance;
    if (clockOutFilters.length > 0) {
      result = result.filter(a => {
        if (clockOutFilters.includes('IN_PROGRESS') && !a.clockOut) return true;
        if (clockOutFilters.includes('CLOCKED_OUT') && !!a.clockOut) return true;
        return false;
      });
    }
    if (shiftFilters.length > 0) {
      result = result.filter(a => {
        if (shiftFilters.includes('SHIFT_1') && (!a.shift || a.shift === 1)) return true;
        if (shiftFilters.includes('SHIFT_2') && a.shift === 2) return true;
        return false;
      });
    }
    return result;
  }, [serverAttendance, clockOutFilters, shiftFilters]);
    ...
    <div className="animate-in fade-in duration-300 space-y-6">
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
        ...
        <div className="flex flex-col gap-2.5">
          ...
          {(() => {
            const activeCount = selectedBranchIds.length + clockOutFilters.length + shiftFilters.length + (dateFrom !== todayStr || dateTo !== todayStr ? 1 : 0);
            return (
              <button
                onClick={() => { setFiltersOpen(o => !o); playSound('click'); }}
                className="sm:hidden w-full h-10 flex items-center justify-between px-4 rounded-2xl border border-slate-200 bg-slate-50 text-slate-500 text-xs font-semibold uppercase tracking-wide transition-all"
              >
                <div className="flex items-center gap-2">
                  <svg className="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M3 4h18M7 8h10M11 12h2"/></svg>
                  <span>Filters{activeCount > 0 ? ` (${activeCount})` : ''}</span>
                </div>
                <svg className={`w-3.5 h-3.5 transition-transform ${filtersOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3"><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7"/></svg>
              </button>
            );
          })()}

          {/* Row 2: Branch · Date · Status · Shift — inline on desktop, collapsible on mobile */}
          <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 sm:grid ${filtersOpen ? 'grid' : 'hidden'}`}>
            ...
            <div className="flex flex-col gap-2">
              <button
                ...
                className={`h-10 w-full flex items-center justify-between px-4 rounded-2xl border transition-all ${
              ...
              {dateFilterOpen && (
                <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2 px-1">
                  <input type="date" value={dateFrom} max={dateTo || undefined} onChange={e => { setDateFrom(e.target.value); setCurrentPage(1); playSound('click'); }} className="h-10 min-w-0 px-3 bg-slate-50 border border-slate-200 rounded-2xl font-bold text-xs uppercase tracking-widest outline-none focus:border-emerald-500 transition-all" />
                  <span className="text-xs font-black text-slate-300 uppercase tracking-widest shrink-0 text-center">to</span>
                  <input type="date" value={dateTo} min={dateFrom || undefined} onChange={e => { setDateTo(e.target.value); setCurrentPage(1); playSound('click'); }} className="h-10 min-w-0 px-3 bg-slate-50 border border-slate-200 rounded-2xl font-bold text-xs uppercase tracking-widest outline-none focus:border-emerald-500 transition-all" />
                </div>
              )}
            </div>

            <CheckboxDropdown
              placeholder="Clock-out Status"
              selected={clockOutFilters}
              onChange={v => { setClockOutFilters(v); setCurrentPage(1); playSound('click'); }}
              options={[
                { value: 'IN_PROGRESS', label: 'In Progress', activeColor: 'bg-amber-500 border-amber-500' },
                { value: 'CLOCKED_OUT', label: 'Clocked Out', activeColor: 'bg-emerald-600 border-emerald-600' },
              ]}
            />

            <CheckboxDropdown
              placeholder="All Shifts"
              selected={shiftFilters}
              onChange={v => { setShiftFilters(v); setCurrentPage(1); playSound('click'); }}
              options={[
                { value: 'SHIFT_1', label: 'Shift 1', activeColor: 'bg-sky-600 border-sky-600' },
                { value: 'SHIFT_2', label: 'Shift 2', activeColor: 'bg-violet-600 border-violet-600' },
              ]}
            />
          </div>
        </div>
                      ...
                      <tr key={log.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-700/50 transition-colors group">
                        ...
                        <td className="pl-5 pr-4 py-4">
                          ...
                          <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                            {log.shift === 2 && (
                              <span className="px-1.5 py-0.5 bg-violet-100 text-violet-700 border border-violet-200 rounded text-[10px] font-black uppercase tracking-widest">Shift 2</span>
                            )}
                            {isRelief && (
