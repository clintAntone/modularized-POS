    ...
    <div className={`${UI_THEME.layout.modalStandard} ${UI_THEME.radius.modal} p-10 text-center border border-slate-100`}>
      ...
      <div className="flex flex-col gap-3 mt-10">
        <button
          onClick={onConfirm}
          disabled={isOpening}
          className={`w-full text-white font-bold py-5 rounded-2xl text-[12px] uppercase tracking-widest shadow-lg active:scale-95 transition-all flex items-center justify-center gap-3 ${isOpen ? 'bg-rose-600 hover:bg-rose-500' : 'bg-emerald-600 hover:bg-emerald-500'}`}
        >
          {isOpening
            ? <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin" />
            : isOpen ? 'Confirm Closure' : 'Confirm Open'}
        </button>
        <button onClick={onCancel} className="w-full text-slate-400 font-bold py-4 rounded-xl text-[11px] uppercase tracking-widest">
          Cancel
        </button>
      </div>
    </div>
