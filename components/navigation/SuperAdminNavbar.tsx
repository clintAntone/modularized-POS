  ...
  const renderTile = useCallback((item: typeof allTabRegistry[0]) => {
    ...
    return (
      <button
        ...
        className={`relative p-3 sm:p-4 rounded-2xl border text-left flex flex-col gap-2.5 transition-all group overflow-hidden min-h-[90px] sm:min-h-[100px] ${
          isActive
      ...
      >
        ...
        {/* Icon */}
        <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${item.color} relative`}>
          {item.icon}
        ...
        <div className="min-w-0">
          <p className={`text-[11px] sm:text-[12px] font-black uppercase tracking-tight leading-none mb-0.5 transition-colors ${isActive ? 'text-emerald-700' : 'text-slate-900 group-hover:text-emerald-700'}`}>
            {item.label}
          </p>
          <p className="text-[8px] sm:text-[9px] font-bold uppercase tracking-widest text-slate-400 truncate">
            {item.desc}
          </p>
        </div>
      </button>
  ...
  return (
    <>
      ...
      {mounted && showMoreModal && createPortal(
        <div
          className="fixed inset-0 z-[1100] flex items-end sm:items-center justify-center p-0 sm:p-6 bg-slate-950/80 backdrop-blur-xl no-print"
          onClick={() => { playSound('click'); setShowMoreModal(false); }}
        >
          <div
            className={`bg-white w-full sm:w-[92vw] sm:max-w-2xl ${UI_THEME.radius.modal} rounded-b-none sm:rounded-b-[28px] shadow-2xl flex flex-col max-h-[90dvh] sm:max-h-[85vh] overflow-hidden`}
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-4 sm:px-7 sm:py-5 border-b border-slate-100 shrink-0">
              <div>
                ...
                <p className="text-[9px] font-bold uppercase tracking-widest text-slate-400 mt-0.5">
                  {starredIds.size > 0 ? `${starredIds.size} favorited · ` : ''}Tap ★ on a tile to favorite
                </p>
              </div>
              <button
                onClick={() => { playSound('click'); setShowMoreModal(false); }}
            ...
            {/* Content */}
            <div className="flex-1 overflow-y-auto no-scrollbar px-4 py-4 sm:px-6 sm:py-5 space-y-5">
