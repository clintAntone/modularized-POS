    ...
    >
      ...
      <div className="max-w-3xl mx-auto w-full relative z-10 flex-1 flex flex-col pt-10 px-4 sm:px-6">
        ...
        <div className="flex-1 space-y-10 pb-32">
          {connectionError && (
            <div className="bg-red-50 border-2 border-red-200 rounded-2xl p-6 animate-in slide-in-from-top-4 duration-500">
          ...
          {searchTerm.toLowerCase().trim() === 'portal' && (
            <button
              ...
              className="w-full bg-slate-950/90 backdrop-blur-md p-8 rounded-3xl text-left group transition-all active:scale-[0.98] border border-slate-800 shadow-xl animate-in zoom-in duration-300 relative overflow-hidden"
            >
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide">Administrative Access</p>
                </div>
                ...
                <div className="grid grid-cols-1 gap-2">
                  {items.map(b => (
                    <button
                      key={b.id}
                      onClick={() => handleNodeSelect(b.id)}
                      className={`w-full backdrop-blur-md rounded-xl text-left transition-all border shadow-sm flex items-center justify-between overflow-hidden relative group cursor-pointer ${
                        b.isEnabled
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4 min-w-0 flex-1 pl-5 pr-3 py-4">
                        ...
                        <div className="min-w-0">
                          <h3 className={`font-bold uppercase text-sm tracking-tight leading-tight truncate transition-colors ${
                          ...
                          <div className="flex items-center gap-1.5 mt-1">
                            <span className="text-xs font-black text-slate-300 font-mono tracking-widest">TRACE-HC{b.id.slice(0,4).toUpperCase()}</span>
                            {b.isEnabled
                              ? <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${b.isOpen ? 'bg-emerald-400' : 'bg-slate-200'}`} />
                              : <span className="text-xs font-black text-amber-400 uppercase tracking-widest">Inactive</span>
                            }
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 pr-4 shrink-0">
                        {!b.isEnabled && (
                          <span className="text-xs font-black text-amber-300 group-hover:text-amber-500 transition-colors uppercase tracking-widest hidden sm:block">Inactive</span>
                        )}
                        {b.isEnabled && (
                          <span className="text-xs font-black text-slate-300 group-hover:text-slate-500 transition-colors uppercase tracking-widest hidden sm:block">Link Terminal</span>
                        )}
