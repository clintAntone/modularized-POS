  ...
  return (
      <div className="relative flex-1" ref={containerRef}>
        <button
            ...
            className={`w-full flex items-center justify-between pl-12 pr-4 py-3 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 ${UI_THEME.radius.card} transition-all duration-300 relative group/btn min-h-[72px] ${isOpen ? 'border-emerald-500 bg-white dark:bg-slate-700 shadow-lg ring-4 ring-emerald-500/5' : 'hover:border-slate-300 dark:hover:border-slate-500 shadow-sm'}`}
        >
          ...
          <div className="flex flex-col items-start min-w-0 pr-3 gap-1">
            <p className="text-xs font-medium text-slate-400 uppercase tracking-wide leading-none mb-0.5">{label}</p>
            <p className="font-bold text-sm text-slate-900 dark:text-slate-100 uppercase tracking-tight truncate w-full leading-none">{value}</p>
          </div>
        ...
        {isOpen && (
            <div className={`absolute z-[110] top-[calc(100%+8px)] left-0 right-0 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-xl overflow-hidden animate-in zoom-in-95 duration-200 max-h-[340px] overflow-y-auto no-scrollbar ring-1 ring-slate-900/5 dark:ring-slate-700 p-2`}>
              {options.map((opt) => {
                ...
                return (
                    <button
                        ...
                        className={`w-full text-left px-5 py-4 text-xs font-bold uppercase tracking-wider transition-all rounded-xl mb-1 last:mb-0 flex items-center justify-between ${isSelected ? 'bg-slate-900 dark:bg-slate-600 text-white shadow-md' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
                    >
                      {opt.label}
...
export const ReportFilters: React.FC<ReportFiltersProps> = ({
  ...
  return (
      <div className={`bg-white dark:bg-slate-800 p-3 sm:p-6 ${UI_THEME.radius.card} shadow-sm border border-slate-200 dark:border-slate-700 space-y-4 sm:space-y-6 no-print`}>
        <div className="flex flex-row items-center justify-between gap-3 sm:gap-4">
          <div className="flex items-center gap-2.5 sm:gap-3">
            ...
            <div className="text-left">
              <h3 className="text-xs sm:text-sm font-black text-slate-900 dark:text-slate-100 uppercase tracking-tighter">Report History</h3>
              <p className="text-xs sm:text-xs font-medium text-slate-400 uppercase tracking-wide">Historical Data Explorer</p>
            </div>
          </div>
          ...
          {isFiltered && (
            <button 
              ...
              className="px-3 py-1.5 bg-rose-50 text-rose-600 rounded-lg text-xs font-semibold uppercase tracking-wide hover:bg-rose-100 transition-all border border-rose-100"
            >
              Clear Filters
            </button>
          )}
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
          <div className="relative flex-1 group">
            ...
            <input
                ...
                className="w-full pl-10 md:pl-14 pr-4 md:pr-6 py-3 md:py-4 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-2xl md:rounded-2xl text-xs md:text-sm font-medium text-slate-900 dark:text-slate-100 uppercase tracking-wide focus:bg-white dark:focus:bg-slate-700 focus:border-emerald-500 focus:ring-8 focus:ring-emerald-500/5 transition-all outline-none shadow-inner placeholder:text-slate-300 dark:placeholder:text-slate-500"
            />
          </div>
        ...
        <div className="animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="flex flex-col lg:flex-row items-stretch gap-4 md:gap-6">
            <div className="relative flex-1">
              <div
                  className="w-full bg-slate-100 dark:bg-slate-700 p-1 rounded-xl md:rounded-2xl flex items-center shadow-inner border border-slate-200/80 dark:border-slate-600 h-full">
                {['daily', 'weekly', 'monthly'].map(v => (
                    <button
                        ...
                        className={`flex-1 px-4 py-2.5 md:px-6 md:py-4 rounded-[16px] md:rounded-xl text-xs md:text-xs font-semibold uppercase tracking-wide transition-all duration-300 ${view === v ? 'bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 shadow-lg scale-[1.02] border border-slate-100 dark:border-slate-600' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300'}`}
                    >
                      {v}
                    </button>
                ))}
            ...
            {(view === 'weekly' || view === 'monthly') && isNetworkView && setShowBreakdown && (
              <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-700 px-6 py-3 rounded-2xl border border-slate-200 dark:border-slate-600 shadow-inner">
                <p className="text-xs font-medium text-slate-400 uppercase tracking-wide">Breakdown by Branch</p>
                <button
