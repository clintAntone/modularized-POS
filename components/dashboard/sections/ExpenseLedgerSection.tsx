...
function MomBadge({ current, prev }: { current: number; prev: number }) {
  ...
  if (prev === 0) return <span className="text-xs font-medium text-slate-400 uppercase tracking-wide">NEW</span>;
  ...
  const up = pct > 0;
  return (
    <span className={`text-xs font-semibold uppercase tracking-wide ${up ? 'text-rose-500' : 'text-emerald-600'}`}>
      {up ? '▲' : '▼'} {up ? '+' : ''}{pct}%
    </span>
  );
}
    ...
    <div className="w-full mx-auto pb-20 space-y-6">
      ...
      <div className={`bg-white p-6 sm:p-8 ${UI_THEME.radius.card} shadow-sm border border-slate-100`}>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-white border border-slate-200 rounded-2xl flex items-center justify-center text-2xl shadow-xl shrink-0">📒</div>
            <div>
              <h2 className="text-lg font-black text-slate-900 uppercase tracking-tighter leading-none">{branch.name.replace('BRANCH - ', '')}</h2>
              <p className="text-xs font-medium text-slate-400 uppercase tracking-wide mt-0.5">Expense Ledger</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        {(Object.entries(CATEGORY_CONFIG) as [string, typeof CATEGORY_CONFIG[keyof typeof CATEGORY_CONFIG]][]).map(([cat, cfg]) => (
          <button
            ...
            className={`p-5 rounded-2xl border-2 text-left transition-all ${categoryFilter === cat ? `${cfg.bg} ${cfg.border} shadow-md` : 'bg-white border-slate-100 hover:border-slate-200'}`}
          >
            <div className="flex items-center justify-between gap-2 mb-2">
              <div className="flex items-center gap-2">
                <span className={`w-2 h-2 rounded-full ${cfg.dot}`}></span>
                <p className={`text-xs font-semibold uppercase tracking-wide ${categoryFilter === cat ? cfg.text : 'text-slate-400'}`}>{cfg.label}</p>
              </div>
        ...
        <div className="p-5 rounded-2xl border-2 border-slate-200 bg-slate-50 text-left">
          <div className="flex items-center justify-between gap-2 mb-2">
            <p className="text-xs font-medium text-slate-400 uppercase tracking-wide">Period Total</p>
      ...
      {periodItems.length > 0 && (() => {
        ...
        return (
          <div className="bg-white rounded-2xl border border-slate-100 p-5 space-y-4">
            <p className="text-xs font-medium text-slate-400 uppercase tracking-wide">Period Highlights</p>
              ...
              {biggestItem && (
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide whitespace-nowrap">Biggest Expense</p>
              ...
              {topName && topCount > 1 && (
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide whitespace-nowrap">Most Frequent</p>
              ...
              {hasLastMonth && (
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide whitespace-nowrap">vs Last Month</p>
                  </div>
      ...
      {periodItems.length === 0 ? (
        <div className={`bg-white p-16 ${UI_THEME.radius.card} border border-slate-100 text-center space-y-3`}>
          ...
          <p className="text-xs font-black text-slate-300 uppercase tracking-wider">No expense records for {periodLabel}.</p>
        </div>
      ) : (
        <>
          {/* ── Mobile Cards ── */}
          <div className="sm:hidden space-y-3">
            {periodItems.map((item, i) => {
              ...
              return (
                <div key={item.id || i} className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    {cfg ? (
                      <span className={`${cfg.bg} ${cfg.text} flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold uppercase tracking-wide`}>
            ...
            <div className="bg-slate-50 rounded-xl border border-slate-200 p-4 flex items-center justify-between">
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide">{periodItems.length} entries</p>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-100">
                    <th className="px-6 sm:px-8 py-4 text-xs font-medium text-slate-400 uppercase tracking-wide w-28">Date</th>
                    <th className="px-4 py-4 text-xs font-medium text-slate-400 uppercase tracking-wide">Name</th>
                    <th className="px-4 py-4 text-xs font-medium text-slate-400 uppercase tracking-wide">Category</th>
                    <th className="px-4 sm:px-8 py-4 text-xs font-medium text-slate-400 uppercase tracking-wide text-right">Amount</th>
                    <th className="px-4 sm:px-8 py-4 text-xs font-medium text-slate-400 uppercase tracking-wide text-center w-16">Proof</th>
                  </tr>
                </thead>
                  ...
                  {periodItems.map((item, i) => {
                    ...
                    return (
                      <tr key={item.id || i} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-4 py-4">
                          {cfg ? (
                            <span className={`${cfg.bg} ${cfg.text} flex items-center gap-1.5 w-fit px-2.5 py-1 rounded-full text-xs font-semibold uppercase tracking-wide`}>
                              <span className={`w-1.5 h-1.5 rounded-full ${cfg.dot}`}></span>
                              {cfg.label}
                            </span>
                          ) : (
                ...
                <tfoot>
                  <tr className="bg-slate-50 border-t-2 border-slate-200">
                    <td colSpan={3} className="px-6 sm:px-8 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wide">
