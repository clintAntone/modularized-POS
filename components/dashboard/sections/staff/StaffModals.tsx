                ...
                {props.isPullMode && isNewStaff && (
                  <div className="space-y-4 animate-in fade-in duration-200">
                    <div className="bg-slate-900 rounded-2xl px-4 py-3.5 flex gap-3 items-start">
                      ...
                      <div>
                        <p className="text-[9px] font-black text-emerald-400 uppercase tracking-widest">Enrolling a Reliever</p>
                        <p className="text-[10px] text-slate-300 mt-1 leading-relaxed">Search for an employee already registered in another branch. They will be temporarily added to this branch's roster. Their home branch record stays unchanged.</p>
                      </div>
                    </div>

                    <div className="space-y-3">
                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1">Search staff from another branch</p>
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full h-11 pl-11 pr-24 bg-slate-50 border-2 border-slate-200 focus:border-emerald-400 focus:bg-white rounded-2xl font-semibold text-[11px] text-slate-700 placeholder:text-slate-300 outline-none transition-all"
                          value={searchQuery}
                        ...
                        <button
                          ...
                          className="absolute right-2 top-1/2 -translate-y-1/2 h-7 px-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl transition-all active:scale-95 font-black text-[9px] uppercase tracking-widest"
                        >
                          Search
                        </button>
                      </div>
                        ...
                        <div className="space-y-1.5 animate-in fade-in slide-in-from-top-2">
                          ...
                          <div className="space-y-1.5 max-h-[280px] overflow-y-auto no-scrollbar">
                            {searchResults.map(emp => (
                              ...
                              >
                                <div className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center text-slate-600 font-black text-[11px] shrink-0 group-hover:bg-emerald-100 group-hover:text-emerald-700 transition-colors overflow-hidden">
                                  {emp.profile
                                    ? <img src={emp.profile} className="w-full h-full object-cover" alt="" />
                                    : getInitials(emp.name)}
                                </div>
                                <div className="flex-1 min-w-0">
                                  <p className="text-[12px] font-black text-slate-900 uppercase truncate">{emp.name}</p>
                    ...
                    <div className="flex items-center gap-3 p-3 bg-emerald-50 border border-emerald-200 rounded-2xl">
                      <div className="w-12 h-12 rounded-xl bg-white border border-emerald-100 flex items-center justify-center shrink-0 overflow-hidden shadow-sm">
                        {props.editingEmployee.profile
                          ...
                          : <span className="text-[12px] font-black text-emerald-600">{getInitials(props.editingEmployee.name || '')}</span>}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-[12px] font-black text-slate-900 uppercase truncate">{props.editingEmployee.name}</p>
                        <div className="flex items-center gap-1 mt-0.5">
                          <MapPin className="w-2.5 h-2.5 text-emerald-400 shrink-0" strokeWidth={2} />
                          <p className="text-[8px] font-bold text-emerald-600 uppercase truncate">
                      ...
                      <button
                        ...
                        className="text-[8px] font-black text-slate-400 uppercase tracking-widest hover:text-rose-500 transition-colors shrink-0 px-2 py-1 rounded-lg hover:bg-rose-50"
                      >
                        Change
                      </button>
                    </div>
