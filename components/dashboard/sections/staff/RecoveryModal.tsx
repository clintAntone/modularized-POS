...
export const RecoveryModal: React.FC<RecoveryModalProps> = ({ employee, branches, isSaving, onClose, onRefresh, onSyncStatusChange, performerName = 'NODE OPERATOR' }) => {
  ...
  const handleCommitReset = async () => {
    ...
    try {
        ...
        if (branchData) {
            const isReliefRole = (employee.role || '').includes('RELIEF');
            const isManagerRole = (employee.role || '').includes('MANAGER') && !isReliefRole;
            const isCurrentlyAssigned = branchData.manager?.toUpperCase() === (employee.name || '').toUpperCase() || 
                                        branchData.tempManager?.toUpperCase() === (employee.name || '').toUpperCase();
            
            if (isCurrentlyAssigned || isManagerRole || isReliefRole) {
                const updates: any = { id: branchData.id, [DB_COLUMNS.IS_PIN_CHANGED]: true };
                if (isManagerRole && branchData.manager?.toUpperCase() !== (employee.name || '').toUpperCase()) {
                    updates[DB_COLUMNS.MANAGER] = (employee.name || '').toUpperCase();
                }
                if (isReliefRole && branchData.tempManager?.toUpperCase() !== (employee.name || '').toUpperCase()) {
                    updates[DB_COLUMNS.TEMP_MANAGER] = (employee.name || '').toUpperCase();
                }
  ...
  return (
    <div className={`${UI_THEME.layout.modalWrapper} no-print`}>
       <div className="bg-white rounded-[44px] w-full max-w-md shadow-xl overflow-hidden animate-in zoom-in duration-300 flex flex-col border border-white/10">
          {successData ? (
            ...
            <>
              ...
              <div className="p-10 space-y-8">
                 <div className="flex items-center gap-4 p-5 bg-rose-50 border border-rose-100 rounded-2xl animate-pulse">
                    <div className="w-10 h-10 rounded-xl bg-rose-600 flex items-center justify-center text-white shadow-lg shrink-0">
                 ...
                 <div className="space-y-6">
                    <div className="space-y-2">
                       <label className="text-xs font-medium text-slate-400 uppercase tracking-wide ml-1">New Terminal Username</label>
                       <input 
                          value={resetUsername} 
                    ...
                    <div className="space-y-2">
                       ...
                       <input 
                          ...
                          className="w-full p-6 bg-slate-50 border-2 border-transparent rounded-xl font-black text-2xl tracking-widest text-center outline-none focus:border-emerald-500 focus:bg-white transition-all shadow-inner tabular-nums" 
                       />
                    </div>
                 </div>
