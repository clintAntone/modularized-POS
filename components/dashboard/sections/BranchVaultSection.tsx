...
export const BranchVaultSection: React.FC<BranchVaultSectionProps> = ({
  ...
  return (
    <div className="w-full mx-auto pb-4 space-y-4 bg-slate-50 dark:bg-slate-700/50 rounded-2xl">
      ...
      {!branch.vaultEnabled && (
        <div className="flex items-center gap-3 px-5 py-3.5 bg-amber-50 border border-amber-200 rounded-2xl">
          ...
          <div>
            <p className="text-xs font-black text-amber-800 uppercase tracking-widest leading-none">Vault Disabled</p>
            <p className="text-xs font-bold text-amber-600 mt-0.5">This vault is archived. Balance and history are read-only.</p>
          </div>
        </div>
      )}
        ...
        {branchVault && (() => {
          ...
          return (
            <div className="space-y-2">
              ...
              {!hasTarget ? (
                <p className="text-xs font-medium uppercase tracking-wide text-slate-400 dark:text-slate-500">Contact admin to set a vault target</p>
              ) : reached ? (
                <p className="text-xs font-semibold uppercase tracking-wide text-emerald-600">Target reached — 100% funded</p>
              ) : (
                <div className="flex items-baseline justify-between gap-2">
                  <p className="text-sm font-black tabular-nums text-slate-800 dark:text-slate-200">₱{remaining.toLocaleString()} <span className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide">to go</span></p>
                  <p className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide">{Math.round(pct * 100)}% of target</p>
                </div>
              )}
              {canCoverBills && (
      ...
      {(() => {
        ...
        return (
          <div className={`bg-white dark:bg-slate-800 ${UI_THEME.radius.card} border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden`}>
            {/* Header */}
            ...
            {/* Search bar */}
            <div className="px-4 sm:px-6 pb-3 pt-1 border-b border-slate-100 dark:border-slate-700">
            ...
            {txLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-14 gap-3 border-t border-slate-50 dark:border-slate-700 px-6">
                <div className="w-12 h-12 rounded-2xl bg-slate-50 dark:bg-slate-700/50 border border-slate-100 dark:border-slate-700 flex items-center justify-center">
                  <Banknote className="w-5 h-5 text-slate-300" />
                </div>
                <div className="text-center space-y-1">
                  <p className="text-sm font-bold text-slate-400 dark:text-slate-500">{billsSearch ? 'No results found' : 'No vault payments yet'}</p>
                  <p className="text-xs text-slate-400 dark:text-slate-500 leading-relaxed max-w-[200px] mx-auto">{billsSearch ? `No matches for "${billsSearch}"` : 'Bills and expenses paid from the vault will appear here'}</p>
                </div>
              </div>
            ) : (
                  ...
                  <div ref={billsBottomRef} className="px-4 py-4 space-y-2">
                    {[1,2,3].map(i => (
                      <div key={i} className="flex items-center gap-3 px-3 py-3 rounded-xl bg-slate-50 dark:bg-slate-700/50 animate-pulse">
                        ...
                        <div className="flex-1 space-y-2">
                          <div className="h-3 bg-slate-200 dark:bg-slate-600 rounded-lg w-2/5" />
                          <div className="h-2 bg-slate-100 dark:bg-slate-700 rounded-lg w-1/4" />
                        </div>
                        <div className="h-4 w-14 bg-rose-100 rounded-lg shrink-0" />
                      </div>
                    ))}
                  </div>
                )}
      ...
      <div className={`bg-white dark:bg-slate-800 ${UI_THEME.radius.card} border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden`}>
        ...
        <button
          ...
          className="w-full px-5 sm:px-6 py-4 flex items-center justify-between gap-3 hover:bg-slate-50 dark:hover:bg-slate-700 active:bg-slate-100 dark:active:bg-slate-700 transition-colors"
        >
          <div className="flex items-center gap-3">
          ...
          <div className={`w-7 h-7 rounded-xl bg-slate-100 dark:bg-slate-700 flex items-center justify-center shrink-0 transition-transform duration-200 ${historyExpanded ? 'rotate-180' : ''}`}>
            <svg className="w-3.5 h-3.5 text-slate-500 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3"><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
          </div>
        </button>
        ...
        {historyExpanded && (
          <>
            {/* Search + filters */}
            <div className="border-b border-slate-100 dark:border-slate-700 space-y-2 pb-3">
              {/* Search bar */}
              <div className="relative px-4 sm:px-6 pt-3">
                <svg className="absolute left-7 sm:left-9 top-1/2 mt-1.5 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 dark:text-slate-500 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                <input
                  type="text"
                  value={historySearch}
                ...
                {historySearch && (
                  <button
                    ...
                    className="absolute right-7 sm:right-9 top-1/2 mt-1.5 -translate-y-1/2 w-4 h-4 flex items-center justify-center rounded-full bg-slate-300 text-white hover:bg-slate-400 transition-colors"
                  >
                    <svg className="w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                  </button>
                )}
            ...
            {txLoading ? (
              ...
              <div className="px-6 py-16 text-center space-y-3">
                <div className="w-12 h-12 rounded-2xl bg-slate-50 dark:bg-slate-700/50 border border-slate-100 dark:border-slate-700 flex items-center justify-center mx-auto">
                ...
                <p className="text-xs text-slate-400 dark:text-slate-500 leading-relaxed max-w-[220px] mx-auto">
                  {historySearch ? `No results for "${historySearch}". Try searching by name or amount.` : filter !== 'all' ? `No ${filter} match your current filters.` : 'Deposits and withdrawals will appear here.'}
                </p>
              </div>
            ) : (
                  ...
                  <div ref={historyBottomRef} className="px-4 py-4 space-y-2">
                    {[1,2,3].map(i => (
                      <div key={i} className="flex items-center gap-3 px-3 py-3 rounded-xl bg-slate-50 dark:bg-slate-700/50 animate-pulse">
                        ...
                        <div className="h-4 w-16 bg-slate-200 dark:bg-slate-600 rounded-lg shrink-0" />
                      </div>
                    ))}
                  </div>
                )}
      ...
      {selectedTx && ReactDOM.createPortal(
        ...
        >
          <div
            className="w-full sm:max-w-sm bg-white dark:bg-slate-800 rounded-t-[32px] sm:rounded-2xl shadow-xl animate-in slide-in-from-bottom-4 sm:zoom-in-95 duration-200 max-h-[90dvh] sm:max-h-[85dvh] flex flex-col"
            onClick={e => e.stopPropagation()}
          >
            {/* Drag handle (mobile only) */}
            <div className="w-10 h-1 bg-slate-200 dark:bg-slate-600 rounded-full mx-auto mt-4 sm:hidden shrink-0" />
            ...
            <div className="p-6 sm:p-8 space-y-5 overflow-y-auto">
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[15px] font-black text-slate-900 dark:text-slate-100 uppercase tracking-tight leading-tight">{selectedTx.name}</p>
                  </div>
                </div>
                <button
                  onClick={() => setSelectedTx(null)}
                  className="w-8 h-8 rounded-xl bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 flex items-center justify-center text-slate-500 dark:text-slate-400 transition-colors shrink-0"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className={`rounded-2xl p-4 flex items-center justify-between ${selectedTx.type === 'deposit' ? 'bg-emerald-50' : 'bg-rose-50'}`}>
                <span className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide">Amount</span>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-slate-50 dark:bg-slate-700 rounded-2xl p-4">
                  <p className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide mb-1">Date</p>
                  <p className="text-xs font-black text-slate-800 dark:text-slate-200 uppercase">{toManilaDate(selectedTx.timestamp)}</p>
                </div>
                <div className="bg-slate-50 dark:bg-slate-700 rounded-2xl p-4">
                  <p className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide mb-1">Time</p>
                  <p className="text-xs font-black text-slate-800 dark:text-slate-200 uppercase">{formatTime(selectedTx.timestamp)}</p>
                </div>
              </div>

              <div className="bg-slate-50 dark:bg-slate-700 rounded-2xl p-4">
                <p className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide mb-1">Reference ID</p>
                <p className="text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-wider">{selectedTx.id.slice(-12).toUpperCase()}</p>
              </div>

              {/* Receipt image */}
              {selectedTx.receiptUrl && (
                <div className="space-y-2">
                  <p className="text-xs font-medium text-slate-400 uppercase tracking-wide">Receipt</p>
                  <a href={selectedTx.receiptUrl} target="_blank" rel="noopener noreferrer" className="block">
                    <img
                      src={selectedTx.receiptUrl}
                      alt="Receipt"
                      className="w-full rounded-2xl object-cover max-h-48 border border-slate-100 dark:border-slate-700 hover:opacity-90 transition-opacity"
                    />
                  </a>
                </div>
              )}
              ...
              <button
                onClick={() => setSelectedTx(null)}
                className="w-full py-4 rounded-2xl border-2 border-slate-200 dark:border-slate-700 text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 active:scale-95 transition-all"
              >
                Close
              </button>
            </div>
      ...
      {billForm !== null && ReactDOM.createPortal(
        ...
        >
          <div
            className="w-full sm:max-w-sm bg-white dark:bg-slate-800 rounded-t-[32px] sm:rounded-2xl shadow-xl animate-in slide-in-from-bottom-4 sm:zoom-in-95 duration-200"
            onClick={e => e.stopPropagation()}
          >
            <div className="w-10 h-1 bg-slate-200 dark:bg-slate-600 rounded-full mx-auto mt-4 sm:hidden" />
            <div className="p-6 sm:p-8 space-y-5">
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide mt-0.5">Monthly recurring bill</p>
                </div>
                <button
                  onClick={() => setBillForm(null)}
                  className="w-8 h-8 rounded-xl bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 flex items-center justify-center text-slate-500 dark:text-slate-400 transition-colors"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              {/* Name */}
              <div>
                <label className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide block mb-2">Bill Name <span className="text-rose-500">*</span></label>
                <input
                  type="text"
                  autoFocus
                  value={billForm.name}
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-700 px-4 py-3 rounded-xl text-xs font-bold uppercase outline-none focus:border-slate-400 focus:bg-white dark:focus:bg-slate-700 dark:text-slate-200 transition-colors"
                />
              </div>

              {/* Amount */}
              <div>
                <label className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide block mb-2">Amount <span className="text-slate-300">(optional)</span></label>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 font-black text-slate-400 dark:text-slate-500">₱</span>
                  <input
                    type="number" min="0" step="1"
                    value={billForm.amount}
                    onChange={e => setBillForm(f => f && ({ ...f, amount: e.target.value }))}
                    placeholder="0"
                    className="w-full bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-700 pl-8 pr-4 py-3 rounded-xl text-sm font-black outline-none focus:border-slate-400 focus:bg-white dark:focus:bg-slate-700 dark:text-slate-200 transition-colors tabular-nums"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide block mb-2">Due Day <span className="text-slate-300">(1–31)</span></label>
                  <input
                    type="number" min="1" max="31" step="1"
                    value={billForm.dueDay}
                    onChange={e => setBillForm(f => f && ({ ...f, dueDay: e.target.value }))}
                    placeholder="e.g. 15"
                    className="w-full bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-700 px-4 py-3 rounded-xl text-sm font-black outline-none focus:border-slate-400 focus:bg-white dark:focus:bg-slate-700 dark:text-slate-200 transition-colors tabular-nums"
                  />
                </div>
                <div>
                  <label className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide block mb-2">Due Month</label>
                  <button
                    type="button"
                    onClick={() => setBillForm(f => f && ({ ...f, dueNextMonth: !f.dueNextMonth }))}
                    className={`w-full py-3 px-4 rounded-xl text-xs font-semibold uppercase tracking-wide border transition-all ${
                      billForm.dueNextMonth
                        ? 'bg-indigo-600 text-white border-indigo-600'
                        : 'bg-slate-50 dark:bg-slate-700/50 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700 hover:border-slate-400'
                    }`}
                  >
              ...
              {/* Notes */}
              <div>
                <label className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide block mb-2">Notes <span className="text-slate-300">(optional)</span></label>
                <input
                  type="text"
                  value={billForm.notes}
                  onChange={e => setBillForm(f => f && ({ ...f, notes: e.target.value }))}
                  placeholder="Any remarks"
                  className="w-full bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-700 px-4 py-3 rounded-xl text-xs font-bold outline-none focus:border-slate-400 focus:bg-white dark:focus:bg-slate-700 dark:text-slate-200 transition-colors"
                />
              </div>

              {billFormError && (
                <p className="text-xs font-bold text-rose-600 bg-rose-50 border border-rose-100 rounded-xl px-3 py-2">{billFormError}</p>
              )}

              <div className="grid grid-cols-2 gap-3">
                <button
                  onClick={() => setBillForm(null)}
                  className="h-12 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-500 dark:text-slate-400 rounded-xl text-xs font-semibold uppercase tracking-wide active:scale-95 transition-all"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="h-12 bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 rounded-xl text-xs font-semibold uppercase tracking-wide active:scale-95 transition-all disabled:opacity-40"
                >
      ...
      {confirmDeleteId !== null && ReactDOM.createPortal(
        ...
        >
          <div
            className="w-full max-w-xs bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-6 space-y-5 animate-in zoom-in-95 duration-200"
            onClick={e => e.stopPropagation()}
          >
            ...
            <div className="text-center">
              <h4 className="text-[15px] font-black text-slate-900 dark:text-slate-100 uppercase tracking-tight">Remove Bill?</h4>
              <p className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide mt-1">This bill will be hidden from the list.</p>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <button
                onClick={() => setConfirmDeleteId(null)}
              ...
              <button
                ...
                className="h-12 bg-rose-600 text-white rounded-xl text-xs font-semibold uppercase tracking-wide active:scale-95 transition-all disabled:opacity-40"
              >
        ...
        >
          ...
          >
            ...
            <div className="p-6 sm:p-8 space-y-5">
              ...
              <div className="flex justify-between items-start">
                <div>
                  <h4 className="text-xl font-black text-slate-900 dark:text-slate-100 uppercase tracking-tight">Deposit to Vault</h4>
                  <p className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide mt-0.5">{branch.name} · Select a report to deduct from</p>
                </div>
                <button
                  onClick={() => { setShowDepositModal(false); setDepositAmount(''); setDepositSelectedDate(null); }}
                  className="w-8 h-8 rounded-xl bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 flex items-center justify-center text-slate-500 dark:text-slate-400 transition-colors shrink-0"
              ...
              <div className="bg-emerald-50 border border-emerald-100 rounded-2xl px-4 py-3 flex items-center justify-between">
                <p className="text-xs font-medium text-slate-500 uppercase tracking-widest">Current Vault Balance</p>
                <p className="text-base font-black text-emerald-700 tabular-nums">₱{(branchVault?.balance ?? 0).toLocaleString()}</p>
              </div>

              {/* Report date picker */}
              <div className="space-y-2">
                <p className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide ml-0.5">Select Report Date (Last 7 Days)</p>
                {currentWeekReports.length === 0 ? (
                  <div className="bg-slate-50 dark:bg-slate-700/50 rounded-2xl p-4 text-center">
                    <p className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide">No reports in the last 7 days</p>
                  </div>
                ) : (
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center justify-between px-4 py-3 rounded-xl border-2 transition-all text-left ${
                            !hasRoi
                              ? 'opacity-40 cursor-not-allowed bg-slate-50 dark:bg-slate-700/50 border-transparent'
                              : isSelected
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <span className={`text-xs font-semibold uppercase tracking-wide ${isSelected ? 'text-indigo-900' : 'text-slate-700 dark:text-slate-300'}`}>{dateLabel}</span>
                              {existingForDate && (
                                <p className="text-xs font-bold text-amber-500 uppercase tracking-widest">Deposited ₱{existingForDate.amount.toLocaleString()} · Tap to update</p>
                              )}
                            </div>
                          </div>
                          <div className="text-right">
                            <span className={`text-xs font-black tabular-nums ${hasRoi ? (isSelected ? 'text-indigo-600' : 'text-emerald-600') : 'text-slate-400'}`}>
                              ₱{roiAvailable.toLocaleString()}
                            </span>
                            <p className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide">Available ROI</p>
                          </div>
                        </button>
                      );
                    })}
                ...
                return (
                  <div className="space-y-3">
                    <div className="space-y-1.5">
                      <label className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide block ml-0.5">Deposit Amount (₱)</label>
                      <div className="relative">
                        <span className="absolute left-3.5 top-1/2 -translate-y-1/2 font-black text-slate-400 dark:text-slate-500">₱</span>
                        <input
                          type="number"
                          autoFocus
                          min="1"
                          ...
                          className={`w-full pl-8 pr-4 py-3 rounded-xl border-2 text-[15px] font-black tabular-nums outline-none transition-all ${
                            isOverMax
                              ...
                              : 'border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700/50 text-indigo-900 dark:text-indigo-300 focus:border-indigo-500 focus:bg-white dark:focus:bg-slate-700'
                          }`}
                        />
                      </div>
                      {isOverMax && (
                        <p className="text-xs font-bold text-rose-500 uppercase tracking-widest ml-0.5">
                          {isTargetCapped
                    ...
                    {amt > 0 && !isOverMax && (
                      <div className="bg-indigo-50 rounded-2xl px-4 py-3 space-y-2">
                        <p className="text-xs font-black text-indigo-400 uppercase tracking-widest">{existingDeposit ? 'After Update' : 'After Deposit'}</p>
                        <div className="flex items-end justify-between">
                          <p className="text-lg font-black text-indigo-700 tabular-nums leading-none">₱{afterBalance.toLocaleString()}</p>
                          {target > 0 && <p className="text-xs font-black text-indigo-500 tabular-nums">{afterPct}% of target</p>}
                        </div>
                        {target > 0 && (
                        ...
                        {existingDeposit ? (
                          <p className="text-xs font-bold text-indigo-400 uppercase tracking-widest tabular-nums">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <button
                  ...
                  className="h-12 bg-indigo-600 text-white rounded-xl text-xs font-semibold uppercase tracking-wide active:scale-95 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  {isSubmittingDeposit
                    ? 'Saving…'
      ...
      {showWithdrawModal && ReactDOM.createPortal(
        ...
        >
          <div
            className="w-full max-w-sm bg-white dark:bg-slate-800 rounded-2xl shadow-xl animate-in zoom-in-95 duration-200 flex flex-col max-h-[92svh]"
            onClick={e => e.stopPropagation()}
          >
            <div className="overflow-y-auto flex-1 p-6 space-y-5">
              ...
              <div className="flex justify-between items-start">
                <div>
                  <h4 className="text-xl font-black text-slate-900 dark:text-slate-100 uppercase tracking-tight">Pay Bill from Vault</h4>
                  <p className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide mt-0.5">{branch.name} · Deduct from vault balance</p>
                </div>
                <button
                  onClick={() => { setShowWithdrawModal(false); setWithdrawLabel(''); setWithdrawAmount(''); setWithdrawFile(null); }}
                  className="w-8 h-8 rounded-xl bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 flex items-center justify-center text-slate-500 dark:text-slate-400 transition-colors shrink-0"
              ...
              {/* Current vault balance */}
              <div className="bg-emerald-50 border border-emerald-100 rounded-2xl px-4 py-3 flex items-center justify-between">
                <p className="text-xs font-medium text-slate-500 uppercase tracking-widest">Available Balance</p>
                <p className="text-base font-black text-emerald-700 tabular-nums">₱{(branchVault?.balance ?? 0).toLocaleString()}</p>
              </div>

              {/* Label */}
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide block ml-0.5">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-700 px-4 py-3 rounded-xl text-xs font-bold uppercase outline-none focus:border-amber-400 focus:bg-white dark:focus:bg-slate-700 dark:text-slate-200 transition-colors placeholder:normal-case placeholder:font-normal"
                />
              </div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          className={`w-full pl-8 pr-4 py-3 rounded-xl border-2 text-[15px] font-black tabular-nums outline-none transition-all ${
                            isOverMax
                              ? 'border-rose-400 bg-rose-50 text-rose-700'
                              : 'border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700/50 text-amber-900 dark:text-amber-300 focus:border-amber-400 focus:bg-white dark:focus:bg-slate-700'
                          }`}
                        />
                      </div>
                      {isOverMax && (
                        <p className="text-xs font-bold text-rose-500 uppercase tracking-widest ml-0.5">Exceeds vault balance of ₱{maxBal.toLocaleString()}</p>
                      )}
                      {amt > 0 && !isOverMax && (
                        <div className="bg-amber-50 rounded-2xl px-4 py-3 flex items-center justify-between">
                          <p className="text-xs font-black text-amber-400 uppercase tracking-widest">Vault after withdrawal</p>
              ...
              {/* Receipt (required) */}
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide block ml-0.5">Receipt <span className="text-rose-500">*</span></label>
                <input
                ...
                {withdrawFile ? (
                  <div className="flex items-center justify-between bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <p className="text-xs font-bold text-amber-700 truncate">{withdrawFile.name}</p>
                    </div>
                    <button
                      type="button"
                  ...
                  <button
                    ...
                    className="w-full py-4 rounded-xl border-2 border-dashed border-rose-200 bg-rose-50/30 text-xs font-bold text-rose-400 uppercase tracking-widest hover:border-rose-400 hover:bg-rose-50 transition-colors flex items-center justify-center gap-2"
                  >
              ...
              {isSubmittingWithdraw && withdrawUploadProgress > 0 && withdrawUploadProgress < 100 && (
                <div className="space-y-1.5">
                  ...
                  <p className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide text-center">
                    Uploading receipt…
                  </p>
                </div>
              )}
              ...
              {withdrawConfirming ? (
                ...
                <div className="space-y-3">
                  <div className="bg-amber-50 border-2 border-amber-200 rounded-2xl overflow-hidden">
                    <div className="px-4 py-2.5 bg-amber-100/60 border-b border-amber-200">
                      <p className="text-xs font-black text-amber-600 uppercase tracking-widest">Confirm Payment</p>
                    </div>
                    <div className="divide-y divide-amber-100">
                      <div className="flex items-center justify-between px-4 py-2.5">
                        <span className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide">Bill</span>
                        <span className="text-xs font-black text-slate-700 dark:text-slate-300 uppercase">{withdrawLabel}</span>
                      </div>
                      <div className="flex items-center justify-between px-4 py-2.5">
                        <span className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide">Amount</span>
                        <span className="text-sm font-black text-amber-600 tabular-nums">−₱{(Number(withdrawAmount) || 0).toLocaleString()}</span>
                      </div>
                      <div className="flex items-center justify-between px-4 py-2.5 bg-slate-50/60 dark:bg-slate-700/40">
                        <span className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide">Vault After</span>
                        <span className="text-xs font-black text-slate-600 dark:text-slate-300 tabular-nums">₱{Math.max(0, (branchVault?.balance ?? 0) - (Number(withdrawAmount) || 0)).toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <button
                      ...
                      className="h-12 bg-amber-500 text-white rounded-xl text-xs font-semibold uppercase tracking-wide active:scale-95 transition-all disabled:opacity-40 flex items-center justify-center gap-2"
                    >
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <button
                    ...
                    className="h-12 bg-amber-500 text-white rounded-xl text-xs font-semibold uppercase tracking-wide active:scale-95 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                    Review Payment
                  </button>
                </div>
      ...
      {vaultBillToDelete && ReactDOM.createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-950/85 backdrop-blur-md animate-in fade-in duration-200 p-4" onClick={() => !isDeletingVaultBill && setVaultBillToDelete(null)}>
          <div className="w-full max-w-sm bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-8 animate-in zoom-in-95 duration-200 text-center" onClick={e => e.stopPropagation()}>
            <div className="w-14 h-14 rounded-2xl bg-rose-50 flex items-center justify-center mx-auto mb-5">
              <Trash2 className="w-6 h-6 text-rose-400" />
            </div>
            <h4 className="text-[18px] font-black text-slate-900 dark:text-slate-100 uppercase tracking-tight mb-1">Reverse Payment?</h4>
            <p className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide mb-5">This will restore the amount to your vault fund</p>
            <div className="bg-slate-50 dark:bg-slate-700/50 rounded-2xl divide-y divide-slate-100 dark:divide-slate-700 mb-6 text-left">
              <div className="flex items-center justify-between px-4 py-2.5">
                <span className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide">Bill</span>
                <span className="text-xs font-black text-slate-700 dark:text-slate-300 uppercase">{vaultBillToDelete.name}</span>
              </div>
              <div className="flex items-center justify-between px-4 py-2.5">
                <span className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide">Refund</span>
                <span className="text-sm font-black text-emerald-600 tabular-nums">+₱{vaultBillToDelete.amount.toLocaleString()}</span>
              </div>
              <div className="flex items-center justify-between px-4 py-2.5">
                <span className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide">Vault After</span>
                <span className="text-xs font-black text-slate-600 dark:text-slate-300 tabular-nums">₱{((branchVault?.balance ?? 0) + vaultBillToDelete.amount).toLocaleString()}</span>
              </div>
            </div>
            <div className="flex flex-col gap-3">
              <button
                ...
                className="w-full py-4 rounded-2xl bg-rose-500 text-white font-black text-xs uppercase tracking-widest active:scale-95 transition-all disabled:opacity-40 flex items-center justify-center gap-2"
              >
              ...
              <button
                ...
                disabled={isDeletingVaultBill}
                className="w-full py-3 rounded-2xl text-slate-400 dark:text-slate-500 font-black text-xs uppercase tracking-widest hover:bg-slate-50 dark:hover:bg-slate-700 transition-all"
              >
                Cancel
              </button>
            </div>
